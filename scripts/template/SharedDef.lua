...
PLAYER_FIELD_MOUNT_BEST = 293	-- PLAYER_FIELD_MOUNT_BEST				= PLAYER_FIELD_DIVINE_FORCE + 1,			//最高坐骑信息(0:ID 1:等级 2：星数)
PLAYER_FIELD_MOUNT_FORCE = 293	-- 坐骑总战力
PLAYER_FIELD_RANK_LIKE = 294
PLAYER_FIELD_USE_RANK_LIKE = 295
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 296	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 297	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 298	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 299	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 300	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 301	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 302	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 326	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 327	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 329	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 331	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 332	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 333	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 334	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 335	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 336
PLAYER_EXPAND_INT_TO_LINE_NO = 337	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 338	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 339	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 340
PLAYER_EXPAND_INT_CLIENT_DATA = 341	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 441	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 448	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 448	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 449	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 473	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 474	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 475	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 483	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 484	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 485	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 487	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 488	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 489	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
