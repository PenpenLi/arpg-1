...
PLAYER_FIELD_RANK_LIKE = 294
PLAYER_FIELD_USE_RANK_LIKE = 295
PLAYER_FIELD_SHOP_LIMIT_START = 296
PLAYER_FIELD_SHOP_LIMIT_END = 316
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 316	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 317	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 318	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 319	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 320	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 321	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 322	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 366	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 367	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 369	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 371	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 372	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 373	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 374	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 375	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 376
PLAYER_EXPAND_INT_TO_LINE_NO = 377	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 378	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 379	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 380
PLAYER_EXPAND_INT_CLIENT_DATA = 381	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 481	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 488	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 488	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 489	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 513	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 514	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 515	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 523	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 524	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 525	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 527	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 528	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 529	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
-- 场景服扩展下标开始了
PLAYER_SCENED_INT_FLAGS = 530	-- 玩家场景服标志位
--  只显示装备的主动技能
PLAYER_INT_FIELD_SPELL_START = 538
PLAYER_INT_FIELD_SPELL_END = 550
-- 重要技能的CD
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_START = 550
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_END = 590
PLAYER_EXPAND_KILL_MONSTER = 590	-- 杀怪数
-- 跨服
PLAYER_APPD_INT_FIELD_KUAFU_WARID = 591	-- 跨服场次id
PLAYER_INT_FIELD_KUAFU_NUMBER = 592	-- 0:玩家报名跨服时收到匹配信息中给的编号 1:跨服类型（值参照枚举：EKUAFU_TYPE）
PLAYER_INT_FIELD_MAIN_QUEST_ID = 593	-- 当前主线任务
PLAYER_INT_FIELD_QUEST_CHAPTER = 594	-- 任务章节奖励领取情况
PLAYER_INT_FIELD_MOUNT_LEVEL = 595	-- 4个bytes(0:当前坐骑阶数 1:当前坐骑星级,2:当前坐骑是否骑乘，3:当前幻化id)
PLAYER_INT_FIELD_DIVINE_ID = 596	-- 当前装备的神兵id
PLAYER_INT_FIELD_PASSIVE_SPELL_START = 597	-- 被动技能开始
PLAYER_INT_FIELD_PASSIVE_SPELL_END = 647	-- 被动技能个数
PLAYER_INT_FIELD_VIRTUAL_CAMP = 647	-- 虚拟阵营
PLAYER_INT_FIELD_WORLD_3V3_SCORE = 648	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TOTAL_SCORE = 649	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TREND_INFO = 650	-- 胜负走势
...
-- 已开启功能的记录
PLAYER_INT_FIELD_OPEN_MENU_INDEX = 657
PLAYER_INT_FIELD_OPEN_MENU_START = 658
PLAYER_INT_FIELD_OPEN_MENU_END = 758
-- ////////////////////////////////////////////////////////////////////////////////////////////////////////
-- 字符串部分
...
MODULE_CHAT_ALL = 0	-- 聊天
MODULE_MAIL_ALL = 0	-- 邮件
MODULE_MAP_REGION = 1	-- 区域
MODULE_MAP_WORLD = 2	-- 世界
MODULE_ACHIEVE_ALL = 0	-- 成就
MODULE_TITLE_ALL = 0	-- 称号
MODULE_STRENGTH_ALL = 0	-- 我要变强
MODULE_SPELL_ALL = 0	-- 技能
MODULE_DIVINE_ALL = 0	-- 神兵
MODULE_FASHION_ALL = 0	-- 时装
MODULE_PET_ALL = 0	-- 宠物
...
MODULE_MAGIC_WEAPON_ALL = 0	-- 法宝
MODULE_CULTIVATION_ALL = 0	-- 修炼
MODULE_WING_ALL = 0	-- 翅膀
...
MODULE_TEAM_ALL = 0	-- 队伍
MODULE_RANK_LOCAL = 1	-- 本服排行
MODULE_RANK_KUAFU = 2	-- 跨服排行
MODULE_COUPLE_ALL = 0	-- 夫妻
MODULE_MASTER_ALL = 0	-- 师徒
MODULE_MALL_GOLD = 1	-- 元宝商城
MODULE_MALL_SCORE = 2	-- 积分商店
MODULE_MALL_RECHARGE = 3	-- 充值
MODULE_VIP_ALL = 0	-- VIP特权
MODULE_STALL_ALL = 0	-- 摊位
MODULE_AUCTION_ALL = 0	-- 拍卖行
