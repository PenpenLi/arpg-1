...
LOG_ITEM_OPER_TYPE_LOGIN_ACTIVITY_REWARD = 37	-- 修炼场掠夺奖励
...
MONEY_CHANGE_LOGIN_ACTIVITY_REWARD = 99	-- 修练场掠夺奖励
MAX_MONEY_CHANGE_TYPE = 100
-- 特殊包裹类型
...
--  buff的类型 计算属性buff 和 周期性 buff有用
BUFF_TYPE_NONE = 0	-- 无
BUFF_TYPE_ADD_HP_PER_RATE = 1	-- 周期性回血百分比
BUFF_TYPE_ADD_DAMAGE_RATE = 2	-- 攻击增强百分比
BUFF_TYPE_ADD_ARMOR_RATE = 3	-- 防御增强百分比
BUFF_TYPE_ADD_CRIT_RATE = 4	-- 暴击增强百分比
...
UNIT_FIELD_BUFF_LEVEL = 53	-- BUFF等级（byte）
UNIT_FIELD_BUFF_CASTER_GUID = 57	-- buff施法者guid
UNIT_FIELD_FORCE = 73	-- 战斗力
UNIT_FIELD_HEALTH = 75	-- 当前生命	
UNIT_FIELD_MAX_HEALTH = 76	-- 最大生命
UNIT_FIELD_DAMAGE = 77	-- 攻击力
UNIT_FIELD_ARMOR = 78	-- 防御力
UNIT_FIELD_HIT = 79	-- 命中
UNIT_FIELD_MISS = 80	-- 闪避
UNIT_FIELD_CRIT = 81	-- 暴击
UNIT_FIELD_TOUGH = 82	-- 坚韧
UNIT_FIELD_ATTACK_SPEED = 83	-- 攻击速度
UNIT_FIELD_MOVE_SPEED = 84	-- 移动速度
UNIT_FIELD_IGNORE_ARMOR = 85	-- 忽视防御
UNIT_FIELD_IGNORE_MISS = 86	-- 忽视闪避
UNIT_FIELD_RECOVERY = 87	-- 生命值回复
UNIT_FIELD_DAMAGE_AMPLIFY_RATE = 88	-- 伤害加深(万分比)
UNIT_FIELD_DAMAGE_RESIST_RATE = 89	-- 伤害减免(万分比)
UNIT_FIELD_DAMAGE_RETURN_RATE = 90	-- 反弹伤害(万分比)
UNIT_FIELD_VAMPIRIC_RATE = 91	-- 吸血光环(万分比)
UNIT_FIELD_RECOVERY_RATE = 92	-- 回复效率(万分比)
UNIT_FIELD_CRIT_RATE = 93	-- 暴击率(万分比)
UNIT_FIELD_CRIT_RESIST_RATE = 94	-- 抗暴率(万分比)
UNIT_FIELD_CRIT_DAM_RATE = 95	-- 暴击伤害倍数(万分比)
UNIT_FIELD_CRIT_RESIST_DAM_RATE = 96	-- 降暴伤害倍数(万分比)
UNIT_FIELD_HIT_RATE = 97	-- 命中率(万分比)
UNIT_FIELD_MISS_RATE = 98	-- 闪避率(万分比)
UNIT_FIELD_STUN_RATE = 99	-- 眩晕
UNIT_FIELD_ROOTS_RATE = 100	-- 定身
UNIT_FIELD_SILENCE_RATE = 101	-- 沉默
UNIT_FIELD_CHAOS_RATE = 102	-- 混乱
UNIT_FIELD_CHARM_RATE = 103	-- 魅惑
UNIT_FIELD_CONTROL_ENHANCE_RATE = 104	-- 控制增强
UNIT_FIELD_CONTROL_RESIST_RATE = 105	-- 控制减免
UNIT_FIELD_ANGER = 106	-- 愤怒值
UNIT_FIELD_USE_RESPAWN_MAPID = 107	-- 使用复活丹的地图id
UNIT_FIELD_PROCESS_TIME = 108	--  进度条读完的时间戳
UNIT_FIELD_PROCESS_SECONDS = 109	--  读条总共需要时间	
UINT_FIELD_BOSS_DATA = 110	-- boss数据byte 0:boss状态 1:boss前一状态 2:boss索引(云游的是这个用途)
UINT_FIELD_BOSS_DATA1 = 111	-- boss数据byte 0:当前坐标x 1:当前坐标y 2:下一坐标x 3:下一坐标y(云游的是这个用途)
UINT_FIELD_VIEW_MODE = 111	-- 是否是观看模式
UINT_FIELD_VIRTUAL_CAMP = 112	-- 虚拟阵营
UINT_FIELD_XIANFU_INFO = 113	-- 跨服仙府的信息4bytes (0:宝箱数量, 1:死亡次数)
UINT_FIELD_VIP_LEVEL = 114	-- VIP等级
UINT_FIELD_VIP_TIME_OUT = 115	-- VIP超时时间
UINT_FIELD_VAMPIRIC = 116	-- 当前战斗的吸血值
UINT_INT_FIELD_ILLUSION = 118	-- 角色幻化
UINT_FIELD_BOOS_CUR_SPELLID = 119	-- 当前正在释放的蓄力技能id
UINT_FIELD_BOOS_CUR_SPELL_TIME = 120	-- 前一个技能结束的时间戳
MAX_UNIT_FIELD = 121
...
PLAYER_INT_FIELD_BUFF_LEVEL_START = 243	-- buff等级 (byte)
PLAYER_INT_FIELD_BUFF_LEVEL_END = 256
PLAYER_FIELD_SKIN = 256	-- 生物变身
PLAYER_FIELD_BYTES_1 = 257	-- 四个字节,分别存放 0怪物分类
PLAYER_FIELD_FORCE = 258	-- 战斗力
PLAYER_FIELD_EQUIPMENT = 260	-- 装备列表，12个32位预留三个
PLAYER_FIELD_CUR_MOUNT = 280	-- 当前坐骑模板id
PLAYER_FIELD_PEACE_MODE_CD = 281	-- 和平模式CD
PLAYER_FIELD_NOTORIETY = 282	-- 2个short(0:战斗模式, 1:恶名值)
PLAYER_FIELD_BYTES_2 = 283	-- 玩家标识,存4个byte[0灰名时间 1付费等级 2免费复活 3玩家称号]
PLAYER_FIELD_BYTES_3 = 284	-- 四个字节,分别存放 0头像,[1发型2可用技能槽数量3下次复活是否有保护buff]
PLAYER_FIELD_BYTES_4 = 285	-- 四个字节,分别存放 (0:频道屏蔽列表,1:预留,2:预留,3:预留)
PLAYER_FIELD_BYTES_5 = 286	-- 0 GM权限, 1, 2 
PLAYER_FIELD_AUTO_HP = 287	-- 大红
PLAYER_FIELD_AUTO_MP = 289	-- 大蓝
PLAYER_FIELD_VIP_LEVEL = 291	-- VIP 等级
PLAYER_FIELD_VIP_TIME_OUT = 292	-- VIP 超时时间
--  挂机设置
PLAYER_FIELD_HOOK = 293	-- 挂机状态
PLAYER_FIELD_HOOK_BYTE0 = 294	-- 0:槽1技能,1:槽2技能,2:槽3技能,3:槽4技能
PLAYER_FIELD_HOOK_BYTE1 = 295	-- 0:神兵技能,1:怒气技能2:使用银两买药,3:银两不足用元宝买药
PLAYER_FIELD_HOOK_SHORT = 296	-- 0:自动吃药百分比,1:自动使用道具id
PLAYER_FIELD_HOOK_BYTE3 = 297	-- 0:回程复活/原地复活,1:自动使用绑银购买复活丹,2:绑银不足用元宝,3:自动出售某种平直及以下装备
PLAYER_FIELD_FLAGS = 298	-- 主玩家下标flags
PLAYER_FIELD_FLAGS_END = 306	-- 预留8个32位flags
PLAYER_FIELD_ONLINE_TIME = 306	-- 总在线时长(分钟为单位)
PLAYER_FIELD_ANGER = 307	-- 怒气改变
PLAYER_FIELD_TRIAL_LAYERS = 308	-- 试炼塔层数
PLAYER_FIELD_DECLINE_CHANNEL_BYTE0 = 309	-- 拒绝频道A(0系统，1帮派，2组队，3世界)
PLAYER_FIELD_DECLINE_CHANNEL_BYTE1 = 310	-- 拒绝频道B(0:附近)
PLAYER_SCENED_INT_FIELD_PLAYER_JUMP_CD = 311	-- 跳跃CD
PLAYER_INT_FIELD_SYSTEM_MAIL_ID = 312	-- 当前领取的系统邮件的id
PLAYER_FIELD_DIVINE_NUM = 313	-- 神兵数量
PLAYER_FIELD_DIVINE_FORCE = 314	-- 神兵总战斗力
PLAYER_FIELD_MOUNT_FORCE = 315	-- 坐骑总战力
PLAYER_FIELD_SKILL_FORCE = 316	-- 技能战力
PLAYER_FILED_GEM_FORCE = 317	-- 宝石战力
PLAYER_FIELD_STRENGTH_FORCE = 318	-- 强化战力
PLAYER_FIELD_RANK_LIKE = 319
PLAYER_FIELD_USE_RANK_LIKE = 320
PLAYER_FIELD_SHOP_LIMIT_START = 321
PLAYER_FIELD_SHOP_LIMIT_END = 341
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 341	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 342	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 343	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 344	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 345	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 346	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 347	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 391	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 392	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 394	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 396	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 397	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 398	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 399	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 400	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 401
PLAYER_EXPAND_INT_TO_LINE_NO = 402	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 403	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 404	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 405
PLAYER_EXPAND_INT_CLIENT_DATA = 406	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 506	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 513	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 513	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 514	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 538	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 539	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 540	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 548	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 549	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 550	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 552	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 553	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 554	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
-- 场景服扩展下标开始了
PLAYER_SCENED_INT_FLAGS = 555	-- 玩家场景服标志位
--  只显示装备的主动技能
PLAYER_INT_FIELD_SPELL_START = 563
PLAYER_INT_FIELD_SPELL_END = 575
-- 重要技能的CD
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_START = 575
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_END = 615
PLAYER_EXPAND_KILL_MONSTER = 615	-- 杀怪数
-- 跨服
PLAYER_APPD_INT_FIELD_KUAFU_WARID = 616	-- 跨服场次id
PLAYER_INT_FIELD_KUAFU_NUMBER = 617	-- 0:玩家报名跨服时收到匹配信息中给的编号 1:跨服类型（值参照枚举：EKUAFU_TYPE）
PLAYER_INT_FIELD_MAIN_QUEST_ID = 618	-- 当前主线任务
PLAYER_INT_FIELD_QUEST_CHAPTER = 619	-- 任务章节奖励领取情况
PLAYER_INT_FIELD_MOUNT_LEVEL = 620	-- 4个bytes(0:当前坐骑阶数 1:当前坐骑星级,2:当前坐骑是否骑乘，3:当前幻化id)
PLAYER_INT_FIELD_DIVINE_ID = 621	-- 当前装备的神兵id
PLAYER_INT_FIELD_PASSIVE_SPELL_START = 622	-- 被动技能开始
PLAYER_INT_FIELD_PASSIVE_SPELL_END = 672	-- 被动技能个数
PLAYER_INT_FIELD_VIRTUAL_CAMP = 672	-- 虚拟阵营
PLAYER_INT_FIELD_WORLD_3V3_SCORE = 673	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TOTAL_SCORE = 674	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TREND_INFO = 675	-- 胜负走势
PLAYER_INT_FIELD_DOUJIANTAI_RANK = 676	-- 斗剑台排名
PLAYER_INT_FIELD_DOUJIANTAI_REFRESH_TIME = 677	-- 最后一次刷新对手时间戳
PLAYER_INT_FIELD_ARREARS = 678	-- 欠费
PLAYER_INT_FIELD_DAILY_QUEST_FINISHED = 680	--  每日任务完成数
PLAYER_INT_FIELD_BAG_SORT_TIME = 681	-- 整理背包时间戳
-- 已开启功能的记录
PLAYER_INT_FIELD_OPEN_MENU_INDEX = 682
PLAYER_INT_FIELD_OPEN_MENU_START = 683
PLAYER_INT_FIELD_OPEN_MENU_END = 783
-- 已购家族商店物品的记录
PLAYER_INT_FIELD_BUYED_FACTION_SHOP = 784	-- 已购家族商店物品开始
PLAYER_INT_FIELD_BUYED_FACTION_SHOP_END = 792	-- 已购家族商店物品结束
-- 记录强制引导ID 
PLAYER_INT_FIELD_GUIDE_ID_LAST = 793	-- 上次完成的引导
PLAYER_INT_FIELD_GUIDE_ID_NOW = 794	-- 当前进行中的引导
-- 修炼场相关
PLAYER_INT_FIELD_CULTIVATION_LAST_PLUNDER_TIME = 795	-- 上次挑战时间戳
PLAYER_INT_FIELD_CULTIVATION_LAST_REFRESH_TIME = 796	-- 上次刷新对手时间戳
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_EXP = 797	-- 当日累计掠夺经验
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_CHEST = 798	-- 当日累计掠夺宝箱数
PLAYER_INT_FIELD_CULTIVATION_TODAY_PURCHASE_COUNT = 799	-- 当日购买次数
PLAYER_INT_FIELD_CULTIVATION_LEFT_PLUNDER_COUNT = 800	-- 当前剩余挑战次数
PLAYER_INT_FIELD_CULTIVATION_PLUNDER_RECORD_INDEX = 801	-- 修练场修炼记录下标
PLAYER_INT_FIELD_CULTIVATION_TOTAL_BEEN_PLUNDER_COUNT = 802	-- 本轮被掠夺成功次数
PLAYER_INT_FIELD_CULTIVATION_TODAY_TOTAL_PLUNDER_COUNT = 803	-- 本日总掠夺次数
PLAYER_INT_FIELD_CULTIVATION_START_TIME = 804	-- 修炼开始时间
PLAYER_INT_FIELD_CULTIVATION_LOST_INFO = 805	-- 修炼宝箱丢失信息
-- 登陆大礼相关
PLAYER_INT_FIELD_LOGIN_ACTIVITY_TOTAL_DAYS = 806	-- 登陆总天数
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_START = 807	-- 领取状态开始
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_END = 809	-- 领取状态结束
--  非强制引导ID
PLAYER_INT_FIELD_OPTIONAL_GUIDE_START = 810	-- 非强制引导开始
PLAYER_INT_FIELD_OPTIONAL_GUIDE_END = 870	--  非强制引导结束
-- ////////////////////////////////////////////////////////////////////////////////////////////////////////
-- 字符串部分
...
-- 试炼塔
TRIAL_INSTANCE_FIELD_ID = 42	--  当前层数
--  桃花迷阵最大刷新次数
