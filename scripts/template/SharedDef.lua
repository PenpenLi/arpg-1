...
MONEY_TYPE_FACTION = 9	-- 帮派贡献
MONEY_TYPE_HONOR = 10	-- 荣誉
MONEY_TYPE_DOUHUN = 11	-- 斗魂
MAX_MONEY_TYPE = 22
-- 基本属性
BASE_ATTR_HP = 0	-- 生命
...
ITEM_OTHER_ATTR_FAIL_TIME = 31	-- 失效时间
ITEM_OTHER_ATTR_STRONG_LV = 32	-- 强化等级
ITEM_OTHER_ATTR_STRONG_FAIL_COUNT = 33	-- 强化失败次数
ITEM_OTHER_ATTR_FORCE = 34	-- 战斗力
-- 技能族公共CD类型
SPELL_CD_BASE = 0
SPELL_CD_SPECIAL = 1
MAX_SPELL_CD_TYPE = 2
...
UNIT_FIELD_ANGER = 118	-- 愤怒值
UNIT_FIELD_USE_RESPAWN_MAPID = 119	-- 使用复活丹的地图id
UNIT_FIELD_PROCESS_TIME = 120	--  进度条读完的时间戳
UNIT_FIELD_PROCESS_SECONDS = 121	--  读条总共需要时间	
UINT_FIELD_BOSS_DATA = 122	-- boss数据byte 0:boss状态 1:boss前一状态 2:boss索引(云游的是这个用途)
UINT_FIELD_BOSS_DATA1 = 123	-- boss数据byte 0:当前坐标x 1:当前坐标y 2:下一坐标x 3:下一坐标y(云游的是这个用途)
UINT_FIELD_VIEW_MODE = 123	-- 是否是观看模式
UINT_FIELD_VIRTUAL_CAMP = 124	-- 虚拟阵营
UINT_FIELD_XIANFU_INFO = 125	-- 跨服仙府的信息4bytes (0:宝箱数量, 1:死亡次数)
UINT_FIELD_VIP_LEVEL = 126	-- VIP等级
UINT_FIELD_VIP_TIME_OUT = 127	-- VIP超时时间
UINT_FIELD_VAMPIRIC = 128	-- 当前战斗的吸血值
UINT_INT_FIELD_ILLUSION = 130	-- 角色幻化
UINT_FIELD_BOOS_CUR_SPELLID = 131	-- 当前正在释放的蓄力技能id
UINT_FIELD_BOOS_CUR_SPELL_TIME = 132	-- 前一个技能结束的时间戳
UINT_FIELD_BOOS_CUR_SPELL_LAST_COUNT = 133	--  持续技能数量
UINT_INT_FIELD_PASSIVE_SPELL_START = 134	-- 被动技能开始
UINT_INT_FIELD_PASSIVE_SPELL_END = 234	-- 被动技能结束
MAX_UNIT_FIELD = 235
GO_STRING_FIELD_CREATED_BY = 4
UNIT_STRING_FIELD_ALIAS_NAME = 5
UNIT_STRING_FIELD_PICK_NAME = 6	-- 正在采集的玩家
MAX_UNIT_STRING_FIELD = 7
-- 游戏对象类型
...
PLAYER_FIELD_ATTR_RESERVE0 = 77	-- 预留属性0
PLAYER_FIELD_ATTR_RESERVE1 = 79	-- 预留属性1
PLAYER_FIELD_ATTR_RESERVE2 = 81	-- 预留属性2
PLAYER_FIELD_ATTR_RESERVE3 = 83	-- 预留属性3
PLAYER_FIELD_ATTR_RESERVE4 = 85	-- 预留属性4
PLAYER_FIELD_ATTR_RESERVE5 = 87	-- 预留属性5
PLAYER_FIELD_ATTR_RESERVE6 = 89	-- 预留属性6
PLAYER_FIELD_ATTR_RESERVE7 = 91	-- 预留属性7
PLAYER_FIELD_BUFF = 93	-- BUFF
PLAYER_FIELD_BUFF_RESERVE = 143	-- BUFF保留参数	
PLAYER_INT_FIELD_BUFF_GIVER_UINT_GUID = 193	-- buff giver的uintGuid
PLAYER_INT_FIELD_BUFF_EFFECT_ID_START = 243	-- buff等级 (byte)
PLAYER_INT_FIELD_BUFF_EFFECT_ID_END = 293
PLAYER_FIELD_SKIN = 293	-- 生物变身
PLAYER_FIELD_BYTES_1 = 294	-- 四个字节,分别存放 0怪物分类
PLAYER_FIELD_FORCE = 295	-- 战斗力
PLAYER_FIELD_EQUIPMENT = 297	-- 装备列表，12个32位预留三个
PLAYER_FIELD_CUR_MOUNT = 317	-- 当前坐骑模板id
PLAYER_FIELD_PEACE_MODE_CD = 318	-- 和平模式CD
PLAYER_FIELD_NOTORIETY = 319	-- 2个short(0:战斗模式, 1:恶名值)
PLAYER_FIELD_BYTES_2 = 320	-- 玩家标识,存4个byte[0灰名时间 1付费等级 2免费复活 3玩家称号]
PLAYER_FIELD_BYTES_3 = 321	-- 四个字节,分别存放 0头像,[1发型2可用技能槽数量3下次复活是否有保护buff]
PLAYER_FIELD_BYTES_4 = 322	-- 四个字节,分别存放 (0:频道屏蔽列表,1:预留,2:预留,3:预留)
PLAYER_FIELD_BYTES_5 = 323	-- 0 GM权限, 1, 2 
PLAYER_FIELD_AUTO_HP = 324	-- 大红
PLAYER_FIELD_AUTO_MP = 326	-- 大蓝
PLAYER_FIELD_VIP_LEVEL = 328	-- VIP 等级
PLAYER_FIELD_VIP_TIME_OUT = 329	-- VIP 超时时间
--  挂机设置
PLAYER_FIELD_HOOK = 330	-- 挂机状态
PLAYER_FIELD_HOOK_BYTE0 = 331	-- 0:槽1技能,1:槽2技能,2:槽3技能,3:槽4技能
PLAYER_FIELD_HOOK_BYTE1 = 332	-- 0:神兵技能,1:怒气技能2:使用银两买药,3:银两不足用元宝买药
PLAYER_FIELD_HOOK_SHORT = 333	-- 0:自动吃药百分比,1:自动使用道具id
PLAYER_FIELD_HOOK_BYTE3 = 334	-- 0:回程复活/原地复活,1:自动使用绑银购买复活丹,2:绑银不足用元宝,3:自动出售某种平直及以下装备
PLAYER_FIELD_FLAGS = 335	-- 主玩家下标flags
PLAYER_FIELD_FLAGS_END = 343	-- 预留8个32位flags
PLAYER_FIELD_ONLINE_TIME = 343	-- 总在线时长(分钟为单位)
PLAYER_FIELD_ANGER = 344	-- 怒气改变
PLAYER_FIELD_TRIAL_LAYERS = 345
PLAYER_INT_FIELD_TRIAL_FINISHED_SECTIONID = 346	-- (已经通关的关卡id)
PLAYER_INT_FIELD_TRIAL_PROCESS = 347	-- (0:已经杀的怪,1:总共需要杀的怪)
PLAYER_FIELD_DECLINE_CHANNEL_BYTE0 = 348	-- 拒绝频道A(0系统，1帮派，2组队，3世界)
PLAYER_FIELD_DECLINE_CHANNEL_BYTE1 = 349	-- 拒绝频道B(0:附近)
PLAYER_SCENED_INT_FIELD_PLAYER_JUMP_CD = 350	-- 跳跃CD
PLAYER_INT_FIELD_SYSTEM_MAIL_ID = 351	-- 当前领取的系统邮件的id
PLAYER_FIELD_DIVINE_NUM = 352	-- 神兵数量
PLAYER_FIELD_DIVINE_FORCE = 353	-- 神兵总战斗力
PLAYER_FIELD_MOUNT_FORCE = 354	-- 坐骑总战力
PLAYER_FIELD_SKILL_FORCE = 355	-- 技能战力
PLAYER_FILED_GEM_FORCE = 356	-- 宝石战力
PLAYER_FIELD_STRENGTH_FORCE = 357	-- 强化战力
PLAYER_FIELD_RANK_LIKE = 358
PLAYER_FIELD_USE_RANK_LIKE = 359
PLAYER_FIELD_SHOP_LIMIT_START = 360
PLAYER_FIELD_SHOP_LIMIT_END = 380
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 380	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 381	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 382	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 383	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 384	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 385	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 386	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 430	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 431	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 433	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 435	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 436	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 437	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 438	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 439	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 440
PLAYER_EXPAND_INT_TO_LINE_NO = 441	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 442	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 443	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 444
PLAYER_EXPAND_INT_CLIENT_DATA = 445	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 545	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 552	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 552	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 553	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 577	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 578	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 579	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 587	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 588	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 589	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 591	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 592	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 593	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
-- 场景服扩展下标开始了
PLAYER_SCENED_INT_FLAGS = 594	-- 玩家场景服标志位
--  只显示装备的主动技能
PLAYER_INT_FIELD_SPELL_START = 602
PLAYER_INT_FIELD_SPELL_END = 614
-- 重要技能的CD
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_START = 614
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_END = 654
PLAYER_EXPAND_KILL_MONSTER = 654	-- 杀怪数
-- 跨服
PLAYER_APPD_INT_FIELD_KUAFU_WARID = 655	-- 跨服场次id
PLAYER_INT_FIELD_KUAFU_NUMBER = 656	-- 0:玩家报名跨服时收到匹配信息中给的编号 1:跨服类型（值参照枚举：EKUAFU_TYPE）
PLAYER_INT_FIELD_MAIN_QUEST_ID = 657	-- 当前主线任务
PLAYER_INT_FIELD_QUEST_CHAPTER = 658	-- 任务章节奖励领取情况
PLAYER_INT_FIELD_MOUNT_LEVEL = 659	-- 4个bytes(0:当前坐骑阶数 1:当前坐骑星级,2:当前坐骑是否骑乘，3:当前幻化id)
PLAYER_INT_FIELD_DIVINE_ID = 660	-- 当前装备的神兵id
PLAYER_INT_FIELD_PASSIVE_SPELL_START = 661	-- 被动技能开始
PLAYER_INT_FIELD_PASSIVE_SPELL_END = 761	-- 被动技能个数
PLAYER_INT_FIELD_VIRTUAL_CAMP = 761	-- 虚拟阵营
PLAYER_INT_FIELD_WORLD_3V3_SCORE = 762	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TOTAL_SCORE = 763	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TREND_INFO = 764	-- 胜负走势
PLAYER_INT_FIELD_DOUJIANTAI_RANK = 765	-- 斗剑台排名
PLAYER_INT_FIELD_DOUJIANTAI_REFRESH_TIME = 766	-- 最后一次刷新对手时间戳
PLAYER_INT_FIELD_ARREARS = 767	-- 欠费
PLAYER_INT_FIELD_DAILY_QUEST_FINISHED = 769	--  每日任务完成数
PLAYER_INT_FIELD_BAG_SORT_TIME = 770	-- 整理背包时间戳
-- 已开启功能的记录
PLAYER_INT_FIELD_OPEN_MENU_INDEX = 771
PLAYER_INT_FIELD_OPEN_MENU_START = 772
PLAYER_INT_FIELD_OPEN_MENU_END = 872
-- 已购家族商店物品的记录
PLAYER_INT_FIELD_BUYED_FACTION_SHOP = 873	-- 已购家族商店物品开始
PLAYER_INT_FIELD_BUYED_FACTION_SHOP_END = 881	-- 已购家族商店物品结束
-- 记录强制引导ID 
PLAYER_INT_FIELD_GUIDE_ID_LAST = 882	-- 上次完成的引导
PLAYER_INT_FIELD_GUIDE_ID_NOW = 883	-- 当前进行中的引导
-- 修炼场相关
PLAYER_INT_FIELD_CULTIVATION_LAST_PLUNDER_TIME = 884	-- 上次挑战时间戳
PLAYER_INT_FIELD_CULTIVATION_LAST_REFRESH_TIME = 885	-- 上次刷新对手时间戳
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_EXP = 886	-- 当日累计掠夺经验
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_CHEST = 887	-- 当日累计掠夺宝箱数
PLAYER_INT_FIELD_CULTIVATION_TODAY_PURCHASE_COUNT = 888	-- 当日购买次数
PLAYER_INT_FIELD_CULTIVATION_LEFT_PLUNDER_COUNT = 889	-- 当前剩余挑战次数
PLAYER_INT_FIELD_CULTIVATION_PLUNDER_RECORD_INDEX = 890	-- 修练场修炼记录下标
PLAYER_INT_FIELD_CULTIVATION_TOTAL_BEEN_PLUNDER_COUNT = 891	-- 本轮被掠夺成功次数
PLAYER_INT_FIELD_CULTIVATION_TODAY_TOTAL_PLUNDER_COUNT = 892	-- 本日总掠夺次数
PLAYER_INT_FIELD_CULTIVATION_START_TIME = 893	-- 修炼开始时间
PLAYER_INT_FIELD_CULTIVATION_LOST_INFO = 894	-- 修炼宝箱丢失信息
-- 登陆大礼相关
PLAYER_INT_FIELD_LOGIN_ACTIVITY_TOTAL_DAYS = 895	-- 登陆总天数
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_START = 896	-- 领取状态开始
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_END = 898	-- 领取状态结束
--  非强制引导ID
PLAYER_INT_FIELD_OPTIONAL_GUIDE_START = 899	-- 非强制引导开始
PLAYER_INT_FIELD_OPTIONAL_GUIDE_END = 959	--  非强制引导结束
PLAYER_INT_FILED_LEAVE_RISK_TIME = 959	-- 离开冒险世界的时间戳
PLAYER_INT_FILED_LEAVE_RISK_PICKED = 960	-- 是否已经领取不在冒险世界的挂机奖励
PLAYER_INT_FILED_FACTION_BUILDING_SPEEDUP_DAILY_COUNT = 961	-- 家族建筑升级加速每日次数
PLAYER_INT_FILED_FACTION_DONATE_GIFT_EXCHANGE_DAILY_COUNT = 962	-- 家族宝库贡献礼物兑换每日次数
PLAYER_INT_FILED_CHARM_POINT = 963	-- 魅力值
PLAYER_INT_FILED_FACTION_GIFT_NEXT_COUNT_ID = 965	-- 玩家礼物历史记录下一个count_id
PLAYER_INT_FILED_FACTION_GIFT_START = 966	-- 玩家礼物历史记录开始
PLAYER_INT_FILED_FACTION_GIFT_END = 1716
PLAYER_INT_FIELD_CREATE_ICON = 1716	-- 待创建的帮派icon
PLAYER_INT_FIELD_FACTION_GIFT_POINT_COUNT = 1717	-- 帮派魅力赠送总记数
PLAYER_INT_FIELD_FACTION_GIFT_SEND_COUNT = 1719	-- 帮派礼物赠送总记数
PLAYER_INT_FIELD_FACTION_GIFT_BEEN_THANK_COUNT = 1720	-- 帮派礼物赠送被女王感谢记数
PLAYER_INT_FIELD_MASS_BOSS_BUYED_TIMES = 1721	-- 已经购买次数
PLAYER_INT_FIELD_MASS_BOSS_TIMES = 1722	-- 全民boss次数
PLAYER_INT_FIELD_MASS_BOSS_CD = 1723	-- 全民boss回复cd(0:表示已满了, 只有次数不满的情况才有cd)
PLAYER_INT_FIELD_TALISMAN_FORCE = 1724	-- 法宝总战力
PLAYER_INT_FIELD_WINGS_FORCE = 1725	-- 神羽总战力
PLAYER_INT_FIELD_GROUP_INSTANCE_CLEAR_FLAG = 1726	-- 组队副本首次通关flag
-- ////////////////////////////////////////////////////////////////////////////////////////////////////////
-- 字符串部分
...
SPELL_INT_FIELD_MOUNT_SPELL_START = 33	-- 进阶技能开始
SPELL_INT_FIELD_MOUNT_SPELL_END = 43	-- 进阶技能结束
SPELL_INT_FIELD_MOUNT_BLESS_EXP = 43	-- 进阶祝福值
SPELL_INT_FIELD_MOUNT_ILLUSION_START = 44	-- 幻化开始
SPELL_INT_FIELD_MOUNT_ILLUSION_END = 164	-- 幻化结束
SPELL_STRENGTHEN_START = 165	-- 强化开始
SPELL_STRENGTHEN_END = 175	-- 强化结束
SPELL_STRENGTHEN_ALLMUL = 176	-- 全身强化标记
SPELL_GEM_START = 177	-- 宝石开始
SPELL_GEM_END = 217	-- 宝石结束
...
FACTION_INT_FIELD_PLAYER_END = 502	-- 帮派申请索引
FACTION_INT_FIELD_APPLY_PLAYER_COUNT_INDEX = 502
FACTION_INT_FIELD_APPLY_PLAYER = 503	-- 玩家申请加入帮派列表
FACTION_INT_FIELD_APPLY_PLAYER_END = 533	-- 帮派申请玩家列表
FACTION_INT_FIELD_EVENT = 533	-- 帮派记录开始
FACTION_INT_FIELD_EVENT_END = 553	-- 帮派记录结束
FACTION_INT_FIELD_EVENT_FALG = 553	-- 帮派记录标志位
FACTION_INT_FIELD_SHOP = 554	-- 商店开始
FACTION_INT_FIELD_SHOP_END = 562	-- 商店结束
FACTION_INT_FIELD_FORCE = 562	-- 帮派战斗力
FACTION_INT_FIELD_MONEY = 564	-- 帮派资产
FACTION_INT_FIELD_LEVEL = 566	-- 帮派等级
FACTION_INT_FIELD_ACTIVITY = 567	-- 帮派活跃度
FACTION_INT_FIELD_RANK = 568	-- 帮派排名
FACTION_INT_FIELD_FLAGS_ID = 569	-- (byte)0:帮派当前选择的旗帜ID (byte)1:神明等级 (uint16)1:灵气
FACTION_INT_FIELD_MINLEV = 570	-- 加入帮派最小等级
FACTION_INT_FIELD_TOKEN_NUM = 571	-- 令牌数量
FACTION_INT_FIELD_TOKEN_POINTS = 572	-- 令牌今日积分进度
FACTION_INT_FIELD_TOKEN_POINTS_COUNT = 573	-- 令牌今日积分完成次数
FACTION_INT_FIELD_TOKEN_TODAY_BUY_COUNT = 574	-- 今日令牌购买次数
FACTION_INT_FIELD_CHALLENGE_BOSS_ID_MAX = 575	-- 已挑战最大boss id
FACTION_INT_FIELD_CHALLENGE_BOSS_ID = 576	-- 当前挑战boss id
FACTION_INT_FIELD_CHALLENGE_BOSS_START_TIME = 577	-- 当前挑战开始时间
FACTION_INT_FIELD_CHALLENGE_BOSS_HP_RATE = 578	-- 当前boss血量万分比
FACTION_INT_FIELD_CHALLENGE_BOSS_POS = 579	-- 当前boss位置 x,y
FACTION_INT_FIELD_PROTECT_TARGET_HP_RATE = 580	-- 当前守护目标血量万分比
FACTION_INT_FIELD_PROTECT_TARGET_POS = 581	-- 当前守护目标位置 x,y
FACTION_INT_FIELD_CHALLENGE_BOSS_DAMAGE_RANK_START = 582	-- 当前boss输出榜开始 int 输出值
FACTION_INT_FIELD_CHALLENGE_BOSS_DAMAGE_RANK_END = 632	-- 当前boss输出榜结束
FACTION_INT_FIELD_CHALLENGE_BOSS_TOTAL_RANK_START = 632	-- 总输出榜开始
FACTION_INT_FIELD_CHALLENGE_BOSS_TOTAL_RANK_END = 682	-- 总输出榜结束
FACTION_INT_FIELD_STOREHOUSE_ITEM_COUNT = 682	-- 宝库道具数量
FACTION_INT_FIELD_STOREHOUSE_RECORD_CURSOR = 683	-- 宝库记录游标
FACTION_INT_FIELD_BUILDING_ID_START = 684	-- 已有建筑id开始
FACTION_INT_FIELD_BUILDING_ID_END = 694	-- 已有建筑id结束
FACTION_INT_FIELD_BUILDING_LVUP_ID = 694	-- 当前正在升级的建筑id
FACTION_INT_FIELD_BUILDING_LVUP_FINISH_TIME = 695	-- 当前升级建筑完成时间戳
FACTION_INT_FIELD_IS_DISSOLVED = 696	-- 是否解散
FACTION_INT_FIELD_GIFT_WEEK_POINT_START = 697	-- 礼物周榜魅力值计数开始
FACTION_INT_FIELD_GIFT_WEEK_POINT_END = 747	-- 礼物周榜魅力值计数结束
FACTION_INT_FIELD_GIFT_UNCHECK_COUNT_START = 747	-- 未处理礼物计数开始 女王未感谢 成员为领取
FACTION_INT_FIELD_GIFT_UNCHECK_COUNT_END = 797	-- 未处理礼物计数结束
FACTION_INT_FIELD_CHARM_POINT = 798	-- 帮派女王魅力值 double
FACTION_INT_FIELD_GIFT_SEND_COUNT_START = 800	-- 今日赠送礼物次数开始
FACTION_INT_FIELD_GIFT_SEND_COUNT_END = 850	-- 今日赠送礼物次数结束
FACTION_INT_FIELD_GIFT_LAST_SEND_TIME = 850	-- 最后赠送的时间
FACTION_INT_FIELD_GIFT_QUEEN_UNCHECK_COUNT = 852	-- 女王未处理礼包数
...
FACTION_DATA_INT_FIELD_GIFT_START = 0	-- 礼物记录int开始
...
FACTION_DATA_INT_FIELD_GIFT_TMP2 = 37500	-- 临时
FACTION_DATA_INT_FIELD_GIFT_END = 42500	-- 礼物记录结束
FACTION_DATA_INT_FIELD_GIFT_RANK_START = 42500	-- 记录排行礼物id开始
FACTION_DATA_INT_FIELD_GIFT_RANK_END = 50000	-- 记录排行礼物id结束
...
INSTANCE_INT_FIELD_ACTIVE_START = 9	-- 活动开始
INSTANCE_INT_FIELD_ACTIVE_END = 59	-- 活动结束
INSTANCE_INT_FIELD_ACTIVE = 59	-- 活动-活跃度
INSTANCE_INT_FIELD_ACTIVE_REWARD = 60	-- 奖励领取状态
INSTANCE_INT_FIELD_3V3_TIMES = 61	-- 3v3次数 已参加次数 购买次数
INSTANCE_INT_FIELD_3V3_DAY_REWARD = 62	-- 3v3每日次数奖励 4个byte
INSTANCE_INT_FIELD_3V3_SEGMENT_TIME = 63	-- 3v3段位 发放时间
INSTANCE_INT_FIELD_XIANFU_DAY_TIMES = 64	-- 每日挑战次数
INSTANCE_INT_FIELD_DOUJIANTAI_CURSOR = 65	-- 斗剑台信息游标
INSTANCE_INT_FIELD_DOUJIANTAI_ENEMY_S = 66	-- 对手名次开始
INSTANCE_INT_FIELD_DOUJIANTAI_ENEMY_E = 69	-- 对手名次结束
INSTANCE_INT_FIELD_DOUJIAN_TIMES = 70	-- 斗剑次数 已挑战次数 购买次数
INSTANCE_INT_FIELD_DOUJIAN_FIGHT_CD = 71	-- 斗剑台挑战CD时间戳
INSTANCE_INT_FIELD_DOUJIAN_FIRST_GET = 72	-- 首胜奖励标记
INSTANCE_INT_FIELD_DOUJIAN_FIRST_REWARD = 73	-- 首胜奖励领取标记
INSTANCE_INT_FIELD_DOUJIAN_COMBATWIN = 74	-- 连胜纪录 纪录 当前
INSTANCE_INT_FIELD_DOUJIAN_COMBATWIN_REWARD = 75	-- 连胜奖励记录
INSTANCE_INT_FIELD_DOUJIAN_MAX_RANK = 76	-- 历史最高纪录
INSTANCE_INT_FIELD_GROUP_INSTANCE_CLEAR_FLAG = 77	-- 每bit记录对应组队副本id首次通关状态 false:未完成 true:已完成
INSTANCE_INT_FIELD_GROUP_INSTANCE_CHALLENGE_COUNT = 78	-- 组队副本剩余挑战次数
INSTANCE_INT_FIELD_GROUP_INSTANCE_BUY_COUNT = 79	-- 当日组队副本挑战次数购买次数
...
GLOBALVALUE_INT_FIELD_WORLD_BOSS_TIMES = 285	-- 世界BOSS次数
GLOBALVALUE_INT_FIELD_WORLD_BOSS_ID = 286	-- 0:世界BOSS类型1, 1:世界BOSS类型2,2:当前选定的类型,3:房间个数
GLOBALVALUE_INT_FIELD_XIANFU_RECORD_CURSOR = 287	-- 仙府开启宝箱的记录游标
GLOBALVALUE_INT_FIELD_GIFT_RANK_WINER_FACTION_FLAG = 288	-- 上周魅力排行第一家族旗子
GLOBALVALUE_INT_FIELD_GIFT_RANK_NEXT_UPDATE_TIME = 289	-- 魅力排行下次更新时间
GLOBALVALUE_INT_FIELD_GIFT_RANK_CUR_ROUND = 290	-- /魅力排行当前轮数
GLOBALVALUE_INT_FIELD_MASS_BOSS_START = 291	-- 全民boss开始
GLOBALVALUE_INT_FIELD_MASS_BOSS_END = 311
...
MAP_INT_FIELD_RESERVE3 = 8	-- 保留字段3
MAP_INT_FIELD_RESERVE4 = 9	-- 保留字段4
MAP_INT_FIELD_QUESTS_START = 10	-- 副本任务开始
MAP_INT_FIELD_QUESTS_END = 34	-- 副本任务结束
MAP_INT_FIELD_QUESTS_PROCESS_START = 34	-- 进度开始
MAP_INT_FIELD_QUESTS_PROCESS_END = 37	-- 进度结束
MAP_INT_FIELD_CREATE_TM = 37	-- 副本创建时间
MAP_INT_FIELD_START_TM = 38	-- 副本开始时间
MAP_INT_FIELD_QUEST_END_TM = 39	-- 副本任务结束时间
MAP_INT_FIELD_END_TM = 40	-- 副本结束时间
MAP_INT_FIELD_INSTANCE_TYPE = 41	-- 副本类型
-- 地图字符串部分
MAP_STR_GENERAL_ID = 4
MAP_STR_REWARD = 5	-- 副本奖励物品字符串
-- 小怪陆续刷
REFRESH_MONSTER_FIELD_ID = 45	-- 2个short(0:当前已经刷的,1:总共需要刷多少怪)
REFRESH_MONSTER_FIELD_INFO_START = 46	-- 怪物信息开始2个short(0:entry,1:level)
...
--  跨服3v3匹配
KUAFU_3V3_FIELDS_INT_INFO_START = 42	--  跨服数据开始
KUAFU_3V3_FIELDS_INT_INFO_END = 72	--  3v3总共6个人
...
--  跨服仙府夺宝匹配
KUAFU_XIANFU_FIELDS_INT_INFO_START = 42	--  跨服数据开始
KUAFU_XIANFU_FIELDS_INT_INFO_END = 112	--  3v3总共6个人
KUAFU_XIANFU_FIELDS_INT_BOSS_INDX = 112	-- 当前的bossid
KUAFU_XIANFU_FIELDS_INT_BOSS_INFO_START = 113	-- BOSS信息开始
KUAFU_XIANFU_FIELDS_INT_BOSS_INFO_END = 128
KUAFU_XIANFU_FIELDS_HARD = 128	-- 副本难度
XIANFU_TEST_DUMMY_COUNT = 129	-- 假人数量
...
-- 跨服组队
KUAFU_GROUP_INSTANCE_FIELDS_HARD = 42	--  难度
KUAFU_GROUP_INSTANCE_FIELDS_PART = 43	--  组队副本的第几部分
KUAFU_GROUP_INSTANCE_INT_FIELDS_PLAYER_INFO_START = 44
KUAFU_GROUP_INSTANCE_INT_FIELDS_PLAYER_INFO_END = 50
KUAFU_GROUP_INSTANCE_STR_FIELDS_PLAYER_INFO_START = 44
KUAFU_GROUP_INSTANCE_STR_FIELDS_PLAYER_INFO_END = 53
DOUJIANTAI_FIELDS_INT_RANK = 42	--  2 shorts (0:自己的排名, 1:对手的排名)
DOUJIANTAI_FIELDS_INT_COMBAT_WIN_INFO = 43	--  2 shorts (0:历史最大连胜, 1:当前连胜)
XIULIANCHANG_FIELDS_INT_RANK = 42	--  敌人斗剑台排行
XIULIANCHANG_FIELDS_INT_ROBOT_ID = 43	--  敌人机器人序号
XIULIANCHANG_FIELDS_INT_LEVEL = 44	-- 敌人等级 结算经验用
...
-- VIP BOSS
VIP_INSTANCE_FIELD_ID = 42	--  0:VIP副本序号,1:副本难度
MAP_MASS_BOSS_INT_FIELD_ID = 42	-- 全民bossid
WORLDBOSS_FIELDS_WAIT_TIME = 42	-- 报名等待时间戳
WORLDBOSS_FIELDS_BORN_TIME = 43	-- 刷新等待时间戳
RES_INSTANCE_FIELD_ID = 42	-- 资源副本的索引
-- 试炼塔
TRIAL_INSTANCE_FIELD_CURSOR = 42	-- 当前已经刷了多少波了
TRIAL_INSTANCE_FIELD_SECTION_ID = 43	-- 关卡id
TRIAL_INSTANCE_FIELD_ORDER = 44	-- 刷怪优先级序号(1个byte存一个数据)
TRIAL_INSTANCE_FIELD_BOSS_REFRESHED = 45	-- 是否已经刷怪
-- 家族场景
FACTION_INSTANCE_FIELD_GUID = 6	--  家族GUID string
--  桃花迷阵最大刷新次数
TAOHUA_REFRESH_TIMES = 10
-- 桃花迷阵地图数据
TAOHUA_INT_FIELD_REFRESH_TIMES = 42	-- 桃花迷阵已经刷新次数
TAOHUA_INT_FIELD_BOSS_CURR_HP = 43	-- BOSS当前血量
TAOHUA_STR_FIELD_REAL_BOSS_GUID = 5	-- BOSS真身GUID
-- 塔的最大层数
TOWER_MAX_FLOOR = 2
-- 塔怪每层击杀数量
TAGUAI_INT_FIELD_FIRST_FLOOR = 42	-- 第一层的信息
-- 战斗信息整形字符型枚举
-- 貌似没必要,随便留一个吧
...
RANK_TYPE_CHARM = 8	-- 魅力排行
MAX_RANK_TYPE = 9
-- 特殊类型排行榜
RANK_TYPE_DOUJIANTAI = 10	-- 斗剑台排行
-- 单个排行榜最大排名个数
...
-- 排行榜
RANK_LIST_CURRENT_OBJ_INT_FIELD_FORCE = 4	-- 战斗力
RANK_LIST_CURRENT_OBJ_INT_FIELD_LEVEL = 6	-- 等级
RANK_LIST_CURRENT_OBJ_INT_FIELD_MONEY = 7	-- 铜钱
RANK_LIST_CURRENT_OBJ_INT_FIELD_ONLINE_TIME = 9	-- 在线时长
RANK_LIST_CURRENT_OBJ_INT_FIELD_TRIAL = 10	-- 试炼塔层数
RANK_LIST_CURRENT_OBJ_INT_FIELD_SHOW = 11	-- 外观列表开始
RANK_LIST_CURRENT_OBJ_INT_FIELD_SHOW_END = 15	-- 外观结束
RANK_LIST_CURRENT_OBJ_INT_FIELD_LIKE = 16	-- 点赞数
...
MODULE_MAIL = 105	-- 邮件
MODULE_MAP = 106	-- 地图
MODULE_ACHIEVE = 107	-- 成就
MODULE_TITLE = 108	-- 称号
MODULE_STRENGTH = 109	-- 我要变强
MODULE_SPELL = 201	-- 技能
MODULE_DIVINE = 202	-- 神兵
MODULE_FASHION = 203	-- 时装
MODULE_PET = 204	-- 宠物
MODULE_MOUNT = 205	-- 坐骑
MODULE_MIX = 206	-- 炼器
MODULE_MAGIC_WEAPON = 207	-- 法宝
MODULE_CULTIVATION = 208	-- 修炼
MODULE_WING = 209	-- 翅膀
MODULE_MOUNT_GET = 210	-- 坐骑
MODULE_SOCIAL = 301	-- 社交
MODULE_FACTION = 302	-- 家族
MODULE_TEAM = 303	-- 队伍
MODULE_RANK = 304	-- 排行榜
MODULE_COUPLE = 305	-- 夫妻
MODULE_MASTER = 306	-- 师徒
MODULE_MALL = 401	-- 商城
MODULE_VIP = 402	-- VIP特权
MODULE_STALL = 403	-- 摊位
MODULE_AUCTION = 404	-- 拍卖行
MODULE_INSTANCE = 501	-- 副本
MODULE_ACTIVITY = 502	-- 活动
MODULE_ARENA = 503	-- 竞技
MODULE_BOSS = 504	-- BOSS
MODULE_PRACTICE = 505	-- 修练场
MODULE_DAILY_TASKS = 506	-- 日常任务
MODULE_WELFARE = 601	-- 福利
MODULE_RECHARGE_FIRST = 602	-- 首充大礼
MODULE_WEEK = 603	-- 登陆大礼
MODULE_RED_PACKET = 604	-- 红包
MODULE_RECHARGE = 701	-- 充值返利
MODULE_TOTAL_RECHARGE = 702	-- 累积充值
MODULE_QUEEN_RANK = 801	-- 女王排行
MODULE_WORLD_RISK = 901	-- 世界冒险
-- 系统模块子页签
MODULE_ROLE_CHAR = 1	-- 角色
MODULE_ROLE_SETTING = 2	-- 设置
MODULE_ROLE_HANG = 3	-- 挂机
MODULE_BAG_ATTR = 1	-- 属性
MODULE_BAG_ITEM = 2	-- 背包
MODULE_BAG_RONGLIAN = 3	-- 熔炼
MODULE_QUEST_MAIN = 1	-- 主线
MODULE_QUEST_EXTENSION = 2	-- 支线
MODULE_QUEST_ACTIVITY = 3	-- 活动
MODULE_QUEST_QIYU = 4	-- 奇遇
MODULE_CHAT_ALL = 1	-- 聊天
MODULE_MAIL_ALL = 1	-- 邮件
MODULE_MAP_REGION = 1	-- 区域
MODULE_MAP_WORLD = 2	-- 世界
MODULE_ACHIEVE_ALL = 1	-- 成就
MODULE_TITLE_ALL = 1	-- 称号
MODULE_STRENGTH_ALL = 1	-- 我要变强
MODULE_SPELL_ALL = 1	-- 技能
MODULE_DIVINE_ALL = 1	-- 神兵
MODULE_FASHION_ALL = 1	-- 时装
MODULE_PET_ALL = 1	-- 宠物
MODULE_MOUNT_UPGRADE = 1	-- 进阶
MODULE_MOUNT_ILLUSION = 2	-- 幻化
MODULE_MIX_STRENGTH = 1	-- 强化
MODULE_MIX_GEM = 2	-- 宝石
MODULE_MIX_WASH = 3	-- 洗练
MODULE_MAGIC_WEAPON_ALL = 1	-- 法宝
MODULE_CULTIVATION_ALL = 1	-- 修炼
MODULE_WING_ALL = 1	-- 翅膀
MODULE_MOUNT_GET_ALL = 1	-- 获得
...
MODULE_FACTION_ACTIVITY = 4	-- 家族活动
MODULE_FACTION_TREASURY = 5	-- 家族宝库
MODULE_FACTION_SHOP = 6	-- 家族商店
MODULE_FACTION_QUEEN = 7	-- 家族女王
MODULE_FACTION_QUEENITEM = 8	-- 礼物赠送
MODULE_TEAM_ALL = 1	-- 队伍
MODULE_RANK_LOCAL = 1	-- 本服排行
MODULE_RANK_KUAFU = 2	-- 跨服排行
MODULE_COUPLE_ALL = 1	-- 夫妻
MODULE_MASTER_ALL = 1	-- 师徒
MODULE_MALL_GOLD = 1	-- 元宝商城
MODULE_MALL_SCORE = 2	-- 积分商店
MODULE_MALL_RECHARGE = 3	-- 充值
MODULE_VIP_ALL = 1	-- VIP特权
MODULE_STALL_ALL = 1	-- 摊位
MODULE_AUCTION_ALL = 1	-- 拍卖行
MODULE_INSTANCE_PLOT = 1	-- 剧情副本
MODULE_INSTANCE_RES = 2	-- 资源副本
MODULE_INSTANCE_TRIAL = 3	-- 试炼塔
MODULE_INSTANCE_VIP = 4	-- VIP副本
MODULE_INSTANCE_TEAM = 5	-- 多人副本
MODULE_ACTIVITY_DAILY = 1	-- 每日活动
MODULE_ARENA_DOUJIANTAI = 1	-- 斗剑台（1V1）
MODULE_ARENA_XIANMO = 2	-- 仙魔道（3V3）
MODULE_ARENA_XIANFU = 3	-- 仙府夺宝
MODULE_ARENA_5V5 = 4	-- 5V5
MODULE_ARENA_10V10 = 5	-- 10V10
MODULE_ARENA_CONTEST = 6	-- 比武大会
MODULE_BOSS_FIELD_BOSS = 1	-- 野外BOSS
MODULE_BOSS_WORLD_BOSS = 2	-- 世界BOSS
MODULE_PRACTICE_EXP = 1	-- 修炼
MODULE_PRACTICE_ATTACK = 2	-- 掠夺
MODULE_DAILY_TASKS_ALL = 1	-- 日常任务
MODULE_WELFARE_FIRST = 1	-- 每日礼包
MODULE_WELFARE_SIGN = 2	-- 每日签到
MODULE_WELFARE_ONLINE = 3	-- 在线礼包
MODULE_WELFARE_RANK = 4	-- 等级有礼
MODULE_WELFARE_BACK = 5	-- 奖励找回
MODULE_RECHARGE_FIRST_ALL = 1	-- 首充大礼
MODULE_WEEK_ALL = 1	-- 七天登陆
MODULE_RED_PACKET_FACTION = 1	-- 家族红包
MODULE_RED_PACKET_WORLD = 2	-- 世界红包
MODULE_RECHARGE_ALL = 1	-- 充值返利
MODULE_TOTAL_RECHARGE_ALL = 1	-- 累积充值
MODULE_QUEEN_RANK_ALL = 1	-- 女王排行
MODULE_WORLD_RISK_MAIN = 1	-- 世界冒险
MODULE_WORLD_RISK_BOSS = 2	-- 世界冒险BOSS挑战
