...
PLAYER_FIELD_TRIAL_LAYERS = 345	-- 试炼塔层数
PLAYER_FIELD_DECLINE_CHANNEL_BYTE0 = 346	-- 拒绝频道A(0系统，1帮派，2组队，3世界)
PLAYER_FIELD_DECLINE_CHANNEL_BYTE1 = 347	-- 拒绝频道B(0:附近)
PLAYER_SCENED_INT_FIELD_PLAYER_JUMP_CD = 348	-- 跳跃CD
PLAYER_INT_FIELD_SYSTEM_MAIL_ID = 349	-- 当前领取的系统邮件的id
PLAYER_FIELD_DIVINE_NUM = 350	-- 神兵数量
PLAYER_FIELD_DIVINE_FORCE = 351	-- 神兵总战斗力
PLAYER_FIELD_MOUNT_FORCE = 352	-- 坐骑总战力
PLAYER_FIELD_SKILL_FORCE = 353	-- 技能战力
PLAYER_FILED_GEM_FORCE = 354	-- 宝石战力
PLAYER_FIELD_STRENGTH_FORCE = 355	-- 强化战力
PLAYER_FIELD_RANK_LIKE = 356
PLAYER_FIELD_USE_RANK_LIKE = 357
PLAYER_FIELD_SHOP_LIMIT_START = 358
PLAYER_FIELD_SHOP_LIMIT_END = 378
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 378	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 379	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 380	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 381	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 382	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 383	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 384	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 428	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 429	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 431	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 433	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 434	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 435	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 436	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 437	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 438
PLAYER_EXPAND_INT_TO_LINE_NO = 439	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 440	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 441	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 442
PLAYER_EXPAND_INT_CLIENT_DATA = 443	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 543	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 550	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 550	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 551	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 575	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 576	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 577	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 585	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 586	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 587	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 589	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 590	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 591	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
-- 场景服扩展下标开始了
PLAYER_SCENED_INT_FLAGS = 592	-- 玩家场景服标志位
--  只显示装备的主动技能
PLAYER_INT_FIELD_SPELL_START = 600
PLAYER_INT_FIELD_SPELL_END = 612
-- 重要技能的CD
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_START = 612
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_END = 652
PLAYER_EXPAND_KILL_MONSTER = 652	-- 杀怪数
-- 跨服
PLAYER_APPD_INT_FIELD_KUAFU_WARID = 653	-- 跨服场次id
PLAYER_INT_FIELD_KUAFU_NUMBER = 654	-- 0:玩家报名跨服时收到匹配信息中给的编号 1:跨服类型（值参照枚举：EKUAFU_TYPE）
PLAYER_INT_FIELD_MAIN_QUEST_ID = 655	-- 当前主线任务
PLAYER_INT_FIELD_QUEST_CHAPTER = 656	-- 任务章节奖励领取情况
PLAYER_INT_FIELD_MOUNT_LEVEL = 657	-- 4个bytes(0:当前坐骑阶数 1:当前坐骑星级,2:当前坐骑是否骑乘，3:当前幻化id)
PLAYER_INT_FIELD_DIVINE_ID = 658	-- 当前装备的神兵id
PLAYER_INT_FIELD_PASSIVE_SPELL_START = 659	-- 被动技能开始
PLAYER_INT_FIELD_PASSIVE_SPELL_END = 709	-- 被动技能个数
PLAYER_INT_FIELD_VIRTUAL_CAMP = 709	-- 虚拟阵营
PLAYER_INT_FIELD_WORLD_3V3_SCORE = 710	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TOTAL_SCORE = 711	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TREND_INFO = 712	-- 胜负走势
PLAYER_INT_FIELD_DOUJIANTAI_RANK = 713	-- 斗剑台排名
PLAYER_INT_FIELD_DOUJIANTAI_REFRESH_TIME = 714	-- 最后一次刷新对手时间戳
PLAYER_INT_FIELD_ARREARS = 715	-- 欠费
PLAYER_INT_FIELD_DAILY_QUEST_FINISHED = 717	--  每日任务完成数
PLAYER_INT_FIELD_BAG_SORT_TIME = 718	-- 整理背包时间戳
-- 已开启功能的记录
PLAYER_INT_FIELD_OPEN_MENU_INDEX = 719
PLAYER_INT_FIELD_OPEN_MENU_START = 720
PLAYER_INT_FIELD_OPEN_MENU_END = 820
-- 已购家族商店物品的记录
PLAYER_INT_FIELD_BUYED_FACTION_SHOP = 821	-- 已购家族商店物品开始
PLAYER_INT_FIELD_BUYED_FACTION_SHOP_END = 829	-- 已购家族商店物品结束
-- 记录强制引导ID 
PLAYER_INT_FIELD_GUIDE_ID_LAST = 830	-- 上次完成的引导
PLAYER_INT_FIELD_GUIDE_ID_NOW = 831	-- 当前进行中的引导
-- 修炼场相关
PLAYER_INT_FIELD_CULTIVATION_LAST_PLUNDER_TIME = 832	-- 上次挑战时间戳
PLAYER_INT_FIELD_CULTIVATION_LAST_REFRESH_TIME = 833	-- 上次刷新对手时间戳
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_EXP = 834	-- 当日累计掠夺经验
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_CHEST = 835	-- 当日累计掠夺宝箱数
PLAYER_INT_FIELD_CULTIVATION_TODAY_PURCHASE_COUNT = 836	-- 当日购买次数
PLAYER_INT_FIELD_CULTIVATION_LEFT_PLUNDER_COUNT = 837	-- 当前剩余挑战次数
PLAYER_INT_FIELD_CULTIVATION_PLUNDER_RECORD_INDEX = 838	-- 修练场修炼记录下标
PLAYER_INT_FIELD_CULTIVATION_TOTAL_BEEN_PLUNDER_COUNT = 839	-- 本轮被掠夺成功次数
PLAYER_INT_FIELD_CULTIVATION_TODAY_TOTAL_PLUNDER_COUNT = 840	-- 本日总掠夺次数
PLAYER_INT_FIELD_CULTIVATION_START_TIME = 841	-- 修炼开始时间
PLAYER_INT_FIELD_CULTIVATION_LOST_INFO = 842	-- 修炼宝箱丢失信息
-- 登陆大礼相关
PLAYER_INT_FIELD_LOGIN_ACTIVITY_TOTAL_DAYS = 843	-- 登陆总天数
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_START = 844	-- 领取状态开始
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_END = 846	-- 领取状态结束
--  非强制引导ID
PLAYER_INT_FIELD_OPTIONAL_GUIDE_START = 847	-- 非强制引导开始
PLAYER_INT_FIELD_OPTIONAL_GUIDE_END = 907	--  非强制引导结束
-- ////////////////////////////////////////////////////////////////////////////////////////////////////////
-- 字符串部分
