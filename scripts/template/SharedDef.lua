...
UNIT_FIELD_MOUNT_LEVEL = 5	-- 2个shorts(0:当前坐骑是否骑乘，1:当前幻化id)
UNIT_FIELD_UINT16_0 = 6	-- 0：精灵模板，1：精灵等级
UNIT_FIELD_UINT16_1 = 7	-- 0：地图ID，1：跨服1v1连胜次数
UNIT_FIELD_UINT16_2 = 8	-- 0:玩家报名跨服时收到匹配信息中给的编号 1:身上宝物采集阵营编号
UNIT_FIELD_FLAGS = 9	-- 精灵身上的标志位
GO_FIELD_FLAGS = 10	-- 游戏对象的一些标识位
GO_FIELD_DATA = 11	-- 动态数据字段,目前保留为4个int
UNIT_FIELD_NPC_FLAG = 15	-- NPC标识
UNIT_FIELD_INSTANCE_ID = 16	-- 地图实例ID
UNIT_FIELD_EQUIPMENT_COAT = 17	-- 上衣模板
UNIT_FIELD_EQUIPMENT_MAIN_WEAPON = 18	-- 主手武器
UNIT_FIELD_BUFF = 19	-- BUFFID
-- 注意了：创建包需要下发的下标放在 UNIT_FIELD_BUFF前面,UNIT_FIELD_BUFF_TM后面的下标走订阅
UNIT_FIELD_BUFF_TM = 27	-- BUFF时间（掩码从这开始）
UNIT_FIELD_BUFF_RESERVE = 35	-- BUFF预留值
UNIT_FIELD_BUFF_LEVEL = 51	-- BUFF等级（byte）
UNIT_FIELD_BUFF_CASTER_GUID = 55	-- buff施法者guid
UNIT_FIELD_FORCE = 71	-- 战斗力
UNIT_FIELD_HEALTH = 72	-- 当前生命	
UNIT_FIELD_MAXHEALTH = 73	-- 最大生命
UNIT_FIELD_DAMAGE = 74	-- 攻击力
UNIT_FIELD_ARMOR = 75	-- 防御力
UNIT_FIELD_HIT = 76	-- 命中
UNIT_FIELD_DODGE = 77	-- 闪避
UNIT_FIELD_CRIT = 78	-- 暴击
UNIT_FIELD_TOUGH = 79	-- 坚韧
UNIT_FIELD_ATTACK_SPEED = 80	-- 攻击速度
UNIT_FIELD_MOVE_SPEED = 81	-- 移动速度
UNIT_FIELD_AMPLIFY_DAMAGE = 82	-- 伤害加深(万分比)
UNIT_FIELD_IGNORE_DEFENSE = 83	-- 忽视防御(万分比)
UNIT_FIELD_DAMAGE_RESIST = 84	-- 伤害减免(万分比)
UNIT_FIELD_DAMAGE_RETURNED = 85	-- 反弹伤害(万分比)
UNIT_FIELD_HIT_RATE = 86	-- 命中率加成(万分比)
UNIT_FIELD_DODGE_RATE = 87	-- 闪避率加成(万分比)
UNIT_FIELD_CRIT_RATE = 88	-- 暴击率加成(万分比)
UNIT_FIELD_CRITICAL_RESIST_RATE = 89	-- 抗暴率加成(万分比)
UNIT_FIELD_DAMAGE_CRIT_MULTIPLE = 90	-- 暴击伤害倍数(万分比)
UNIT_FIELD_RESIST_CRIT_MULTIPLE = 91	-- 降暴伤害倍数(万分比)
UNIT_FIELD_ANGER = 92	-- 愤怒值
UINT_FIELD_BOSS_DATA = 93	-- boss数据byte 0:boss状态 1:boss前一状态 2:boss索引(云游的是这个用途)
UINT_FIELD_BOSS_DATA1 = 94	-- boss数据byte 0:当前坐标x 1:当前坐标y 2:下一坐标x 3:下一坐标y(云游的是这个用途)
MAX_UNIT_FIELD = 95
...
-- 游戏对象数据
-- 传送点专用
GO_TELETEPORTER_TO_MAPID = 11
GO_TELETEPORTER_TO_X = 12
GO_TELETEPORTER_TO_Y = 13
-- 任务相关游戏对象
GO_QUEST_ID_0 = 11	-- 相关任务0
GO_QUEST_ID_1 = 12	-- 相关任务1
GO_QUEST_ID_2 = 13	-- 相关任务2
GO_QUEST_ID_3 = 14	-- 相关任务3
GO_STRING_FIELD_CREATED_NAME = 5	-- 战利品创建者Name
-- 机关类相关的
GO_GEAR_STATUS = 11	-- 机关状态
GO_GEAR_KILLER = 12	-- 机关破坏者
-- 机关状态枚举
...
PLAYER_FIELD_BYTES_2 = 258	-- 玩家标识,存4个byte[0灰名时间 1付费等级 2免费复活 3玩家称号]
PLAYER_FIELD_BYTES_3 = 259	-- 四个字节,分别存放 0头像,[1发型2可用技能槽数量3攻击模式]
PLAYER_FIELD_BYTES_4 = 260	-- 四个字节,分别存放 0坐骑槽数量,1当前骑乘坐骑的坐骑槽位置，未骑乘值为255,2披风格挡等级，3剩余格挡次数
PLAYER_FIELD_BYTES_5 = 261	-- 0 GM权限, 1, 2 
PLAYER_FIELD_AUTO_HP = 262	-- 大红
PLAYER_FIELD_AUTO_MP = 264	-- 大蓝
PLAYER_FIELD_VIP1_TIME_OUT = 266	-- VIP1 超时时间
PLAYER_FIELD_VIP2_TIME_OUT = 267	-- VIP2 超时时间
PLAYER_FIELD_VIP3_TIME_OUT = 268	-- VIP3 超时时间
PLAYER_FIELD_FLAGS = 269	-- 主玩家下标flags
PLAYER_FIELD_FLAGS_END = 277	-- 预留8个32位flags
PLAYER_FIELD_ONLINE_TIME = 277	-- 总在线时长(分钟为单位)
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 278	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 279	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 280	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 281	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 282	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 283	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 284	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 308	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 309	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 311	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 313	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 314	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 315	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 316	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 317	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 318
PLAYER_EXPAND_INT_TO_LINE_NO = 319	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 320	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 321	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 322
PLAYER_EXPAND_INT_CLIENT_DATA = 323	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 423	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 430	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 430	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 431	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 455	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 456	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 457	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 465	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 466	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 467	-- 主包裹扩展剩余多少时间
...
PLAYER_INT_FIELD_MOUNT_LEVEL = 565	-- 2个shorts(0:当前坐骑是否骑乘，1:当前幻化id)
PLAYER_INT_FIELD_DIVINE_ID = 566	-- 当前装备的神兵id
PLAYER_INT_FIELD_DIVINE_INITIATIVE = 567	-- 神兵主动技能信息
PLAYER_INT_FIELD_DIVINE_PASSIVE_START = 568	-- 神兵被动技能开始
PLAYER_INT_FIELD_DIVINE_PASSIVE_END = 571	-- 神兵被动技能结束
PLAYER_INT_FIELD_PASSIVE_SPELL_COUNT = 571	-- 被动技能的个数(暂时没用)
PLAYER_INT_FIELD_PASSIVE_SPELL_START = 572	-- 被动技能开始
-- ////////////////////////////////////////////////////////////////////////////////////////////////////////
-- 字符串部分
...
SPELL_INT_FIELD_MOUNT_ILLUSION_COUNT = 44	-- 幻化数量
SPELL_INT_FIELD_MOUNT_ILLUSION_START = 45	-- 幻化开始
SPELL_INT_FIELD_MOUNT_ILLUSION_END = 165	-- 幻化结束
SPELL_DIVINE_COUNT = 165	-- 神兵个数
SPELL_INT_FIELD_DIVINE_START = 166	-- 神兵开始
SPELL_INT_FIELD_DIVINE_END = 216
-- 需要发内部消息同步到场景服的技能类型
