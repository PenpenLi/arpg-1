...
UNIT_FIELD_HEALTH = 74	-- 当前生命	
UNIT_FIELD_MAXHEALTH = 75	-- 最大生命
UNIT_FIELD_DAMAGE = 76	-- 攻击力
UNIT_FIELD_ARMOR = 77	-- 防御力
UNIT_FIELD_HIT = 78	-- 命中
UNIT_FIELD_DODGE = 79	-- 闪避
UNIT_FIELD_CRIT = 80	-- 暴击
UNIT_FIELD_TOUGH = 81	-- 坚韧
UNIT_FIELD_ATTACK_SPEED = 82	-- 攻击速度
UNIT_FIELD_MOVE_SPEED = 83	-- 移动速度
UNIT_FIELD_AMPLIFY_DAMAGE = 84	-- 伤害加深(万分比)
UNIT_FIELD_IGNORE_DEFENSE = 85	-- 忽视防御(万分比)
UNIT_FIELD_DAMAGE_RESIST = 86	-- 伤害减免(万分比)
UNIT_FIELD_DAMAGE_RETURNED = 87	-- 反弹伤害(万分比)
UNIT_FIELD_HIT_RATE = 88	-- 命中率加成(万分比)
UNIT_FIELD_DODGE_RATE = 89	-- 闪避率加成(万分比)
UNIT_FIELD_CRIT_RATE = 90	-- 暴击率加成(万分比)
UNIT_FIELD_CRITICAL_RESIST_RATE = 91	-- 抗暴率加成(万分比)
UNIT_FIELD_DAMAGE_CRIT_MULTIPLE = 92	-- 暴击伤害倍数(万分比)
UNIT_FIELD_RESIST_CRIT_MULTIPLE = 93	-- 降暴伤害倍数(万分比)
UNIT_FIELD_ANGER = 94	-- 愤怒值
UINT_FIELD_BOSS_DATA = 95	-- boss数据byte 0:boss状态 1:boss前一状态 2:boss索引(云游的是这个用途)
UINT_FIELD_BOSS_DATA1 = 96	-- boss数据byte 0:当前坐标x 1:当前坐标y 2:下一坐标x 3:下一坐标y(云游的是这个用途)
MAX_UNIT_FIELD = 97
...
PLAYER_SCENED_INT_FIELD_PLAYER_JUMP_CD = 281	-- 跳跃CD
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 282	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 283	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 284	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 285	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 286	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 287	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 288	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 312	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 313	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 315	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 317	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 318	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 319	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 320	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 321	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 322
PLAYER_EXPAND_INT_TO_LINE_NO = 323	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 324	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 325	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 326
PLAYER_EXPAND_INT_CLIENT_DATA = 327	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 427	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 434	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 434	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 435	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 459	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 460	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 461	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 469	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 470	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 471	-- 主包裹扩展剩余多少时间
