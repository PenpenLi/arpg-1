...
EQUIP_ATTR_MAXHEALTH = 1	-- 生命
EQUIP_ATTR_DAMAGE = 2	-- 攻击
EQUIP_ATTR_ARMOR = 3	-- 防御
EQUIP_ATTR_HIT = 4	-- 命中
EQUIP_ATTR_DODGE = 5	-- 闪避
EQUIP_ATTR_CRIT = 6	-- 暴击
EQUIP_ATTR_TOUGH = 7	-- 坚韧
EQUIP_ATTR_ATTACK_SPEED = 8	-- 攻速
EQUIP_ATTR_MOVE_SPEED = 9	-- 移速
EQUIP_ATTR_AMPLIFY_DAMAGE = 10	-- 伤害加深(万分比)
EQUIP_ATTR_IGNORE_DEFENSE = 11	-- 忽视防御(万分比)
EQUIP_ATTR_DAMAGE_RESIST = 12	-- 伤害减免(万分比)
EQUIP_ATTR_DAMAGE_RETURNED = 13	-- 反弹伤害(万分比)
EQUIP_ATTR_HIT_RATE = 14	-- 命中率加成(万分比)
EQUIP_ATTR_DODGE_RATE = 15	-- 闪避率加成(万分比)
EQUIP_ATTR_CRIT_RATE = 16	-- 暴击率加成(万分比)
EQUIP_ATTR_CRITICAL_RESIST_RATE = 17	-- 抗暴率加成(万分比)
EQUIP_ATTR_DAMAGE_CRIT_MULTIPLE = 18	-- 暴击伤害倍数(万分比)
EQUIP_ATTR_RESIST_CRIT_MULTIPLE = 19	-- 降暴伤害倍数(万分比)
MAX_EQUIP_ATTR = 20
-- 物品其他属性枚举（物品特殊属性接在MAX_EQUIP_ATTR后面）
ITEM_OTHER_ATTR_FAIL_TIME = 20	-- 失效时间
...
UNIT_FIELD_MAXHEALTH = 76	-- 最大生命
UNIT_FIELD_DAMAGE = 77	-- 攻击力
UNIT_FIELD_ARMOR = 78	-- 防御力
UNIT_FIELD_HIT = 79	-- 命中
UNIT_FIELD_DODGE = 80	-- 闪避
UNIT_FIELD_CRIT = 81	-- 暴击
UNIT_FIELD_TOUGH = 82	-- 坚韧
UNIT_FIELD_ATTACK_SPEED = 83	-- 攻击速度
UNIT_FIELD_MOVE_SPEED = 84	-- 移动速度
UNIT_FIELD_AMPLIFY_DAMAGE = 85	-- 伤害加深(万分比)
UNIT_FIELD_IGNORE_DEFENSE = 86	-- 忽视防御(万分比)
UNIT_FIELD_DAMAGE_RESIST = 87	-- 伤害减免(万分比)
UNIT_FIELD_DAMAGE_RETURNED = 88	-- 反弹伤害(万分比)
UNIT_FIELD_HIT_RATE = 89	-- 命中率加成(万分比)
UNIT_FIELD_DODGE_RATE = 90	-- 闪避率加成(万分比)
UNIT_FIELD_CRIT_RATE = 91	-- 暴击率加成(万分比)
UNIT_FIELD_CRITICAL_RESIST_RATE = 92	-- 抗暴率加成(万分比)
UNIT_FIELD_DAMAGE_CRIT_MULTIPLE = 93	-- 暴击伤害倍数(万分比)
UNIT_FIELD_RESIST_CRIT_MULTIPLE = 94	-- 降暴伤害倍数(万分比)
UNIT_FIELD_ANGER = 95	-- 愤怒值
UNIT_FIELD_USE_RESPAWN_MAPID = 96	-- 使用复活丹的地图id
UNIT_FIELD_PROCESS_TIME = 97	--  进度条读完的时间戳
UNIT_FIELD_PROCESS_SECONDS = 98	--  读条总共需要时间	
UINT_FIELD_BOSS_DATA = 99	-- boss数据byte 0:boss状态 1:boss前一状态 2:boss索引(云游的是这个用途)
UINT_FIELD_BOSS_DATA1 = 100	-- boss数据byte 0:当前坐标x 1:当前坐标y 2:下一坐标x 3:下一坐标y(云游的是这个用途)
UINT_FIELD_VIEW_MODE = 100	-- 是否是观看模式
UINT_FIELD_VIRTUAL_CAMP = 101	-- 虚拟阵营
UINT_FIELD_XIANFU_INFO = 102	-- 跨服仙府的信息4bytes (0:宝箱数量, 1:死亡次数)
UINT_FIELD_VIP_LEVEL = 103	-- VIP等级
UINT_FIELD_VIP_TIME_OUT = 104	-- VIP超时时间
UINT_INT_FIELD_ILLUSION = 105	-- 角色幻化
MAX_UNIT_FIELD = 106
...
PLAYER_FIELD_MAXHEALTH = 17	-- 最大生命
PLAYER_FIELD_DAMAGE = 19	-- 攻击力
PLAYER_FIELD_ARMOR = 21	-- 防御力
PLAYER_FIELD_HIT = 23	-- 命中
PLAYER_FIELD_DODGE = 25	-- 闪避
PLAYER_FIELD_CRIT = 27	-- 暴击
PLAYER_FIELD_TOUGH = 29	-- 坚韧
PLAYER_FIELD_ATTACK_SPEED = 31	-- 攻击速度
PLAYER_FIELD_MOVE_SPEED = 33	-- 移动速度
PLAYER_FIELD_AMPLIFY_DAMAGE = 35	-- 伤害加深(万分比)
PLAYER_FIELD_IGNORE_DEFENSE = 37	-- 忽视防御(万分比)
PLAYER_FIELD_DAMAGE_RESIST = 39	-- 伤害减免(万分比)
PLAYER_FIELD_DAMAGE_RETURNED = 41	-- 反弹伤害(万分比)
PLAYER_FIELD_HIT_RATE = 43	-- 命中率加成(万分比)
PLAYER_FIELD_DODGE_RATE = 45	-- 闪避率加成(万分比)
PLAYER_FIELD_CRIT_RATE = 47	-- 暴击率加成(万分比)
PLAYER_FIELD_CRITICAL_RESIST_RATE = 49	-- 抗暴率加成(万分比)
PLAYER_FIELD_DAMAGE_CRIT_MULTIPLE = 51	-- 暴击伤害倍数(万分比)
PLAYER_FIELD_RESIST_CRIT_MULTIPLE = 53	-- 降暴伤害倍数(万分比)
PLAYER_FIELD_ATTR_RESERVE0 = 55	-- 预留属性0
PLAYER_FIELD_ATTR_RESERVE1 = 57	-- 预留属性1
PLAYER_FIELD_ATTR_RESERVE2 = 59	-- 预留属性2
PLAYER_FIELD_ATTR_RESERVE3 = 61	-- 预留属性3
PLAYER_FIELD_ATTR_RESERVE4 = 63	-- 预留属性4
PLAYER_FIELD_ATTR_RESERVE5 = 65	-- 预留属性5
PLAYER_FIELD_ATTR_RESERVE6 = 67	-- 预留属性6
PLAYER_FIELD_ATTR_RESERVE7 = 69	-- 预留属性7
PLAYER_FIELD_BUFF = 71	-- BUFF
PLAYER_FIELD_BUFF_RESERVE = 121	-- BUFF保留参数	
PLAYER_INT_FIELD_BUFF_GIVER_UINT_GUID = 171	-- buff giver的uintGuid
PLAYER_INT_FIELD_BUFF_LEVEL_START = 221	-- buff等级 (byte)
PLAYER_INT_FIELD_BUFF_LEVEL_END = 234
PLAYER_FIELD_SKIN = 234	-- 生物变身
PLAYER_FIELD_BYTES_1 = 235	-- 四个字节,分别存放 0怪物分类
PLAYER_FIELD_FORCE = 236	-- 战斗力
PLAYER_FIELD_EQUIPMENT = 238	-- 装备列表，12个32位预留三个
PLAYER_FIELD_CUR_MOUNT = 258	-- 当前坐骑模板id
PLAYER_FIELD_PEACE_MODE_CD = 259	-- 和平模式CD
PLAYER_FIELD_NOTORIETY = 260	-- 2个short(0:战斗模式, 1:恶名值)
PLAYER_FIELD_BYTES_2 = 261	-- 玩家标识,存4个byte[0灰名时间 1付费等级 2免费复活 3玩家称号]
PLAYER_FIELD_BYTES_3 = 262	-- 四个字节,分别存放 0头像,[1发型2可用技能槽数量3下次复活是否有保护buff]
PLAYER_FIELD_BYTES_4 = 263	-- 四个字节,分别存放 (0:频道屏蔽列表,1:预留,2:预留,3:预留)
PLAYER_FIELD_BYTES_5 = 264	-- 0 GM权限, 1, 2 
PLAYER_FIELD_AUTO_HP = 265	-- 大红
PLAYER_FIELD_AUTO_MP = 267	-- 大蓝
PLAYER_FIELD_VIP_LEVEL = 269	-- VIP 等级
PLAYER_FIELD_VIP_TIME_OUT = 270	-- VIP 超时时间
--  挂机设置
PLAYER_FIELD_HOOK = 271	-- 挂机状态
PLAYER_FIELD_HOOK_BYTE0 = 272	-- 0:槽1技能,1:槽2技能,2:槽3技能,3:槽4技能
PLAYER_FIELD_HOOK_BYTE1 = 273	-- 0:神兵技能,1:怒气技能2:使用银两买药,3:银两不足用元宝买药
PLAYER_FIELD_HOOK_SHORT = 274	-- 0:自动吃药百分比,1:自动使用道具id
PLAYER_FIELD_HOOK_BYTE3 = 275	-- 0:回程复活/原地复活,1:自动使用绑银购买复活丹,2:绑银不足用元宝,3:自动出售某种平直及以下装备
PLAYER_FIELD_FLAGS = 276	-- 主玩家下标flags
PLAYER_FIELD_FLAGS_END = 284	-- 预留8个32位flags
PLAYER_FIELD_ONLINE_TIME = 284	-- 总在线时长(分钟为单位)
PLAYER_FIELD_ANGER = 285	-- 怒气改变
PLAYER_FIELD_TRIAL_LAYERS = 286	-- 试炼塔层数
PLAYER_FIELD_DECLINE_CHANNEL_BYTE0 = 287	-- 拒绝频道A(0系统，1帮派，2组队，3世界)
PLAYER_FIELD_DECLINE_CHANNEL_BYTE1 = 288	-- 拒绝频道B(0:附近)
PLAYER_SCENED_INT_FIELD_PLAYER_JUMP_CD = 289	-- 跳跃CD
PLAYER_INT_FIELD_SYSTEM_MAIL_ID = 290	-- 当前领取的系统邮件的id
PLAYER_FIELD_DIVINE_NUM = 291	-- 神兵数量
PLAYER_FIELD_DIVINE_FORCE = 292	-- 神兵总战斗力
PLAYER_FIELD_MOUNT_FORCE = 293	-- 坐骑总战力
PLAYER_FIELD_SKILL_FORCE = 294	-- 技能战力
PLAYER_FILED_GEM_FORCE = 295	-- 宝石战力
PLAYER_FIELD_STRENGTH_FORCE = 296	-- 强化战力
PLAYER_FIELD_RANK_LIKE = 297
PLAYER_FIELD_USE_RANK_LIKE = 298
PLAYER_FIELD_SHOP_LIMIT_START = 299
PLAYER_FIELD_SHOP_LIMIT_END = 319
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 319	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 320	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 321	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 322	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 323	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 324	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 325	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 369	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 370	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 372	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 374	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 375	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 376	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 377	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 378	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 379
PLAYER_EXPAND_INT_TO_LINE_NO = 380	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 381	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 382	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 383
PLAYER_EXPAND_INT_CLIENT_DATA = 384	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 484	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 491	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 491	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 492	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 516	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 517	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 518	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 526	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 527	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 528	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 530	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 531	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 532	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
-- 场景服扩展下标开始了
PLAYER_SCENED_INT_FLAGS = 533	-- 玩家场景服标志位
--  只显示装备的主动技能
PLAYER_INT_FIELD_SPELL_START = 541
PLAYER_INT_FIELD_SPELL_END = 553
-- 重要技能的CD
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_START = 553
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_END = 593
PLAYER_EXPAND_KILL_MONSTER = 593	-- 杀怪数
-- 跨服
PLAYER_APPD_INT_FIELD_KUAFU_WARID = 594	-- 跨服场次id
PLAYER_INT_FIELD_KUAFU_NUMBER = 595	-- 0:玩家报名跨服时收到匹配信息中给的编号 1:跨服类型（值参照枚举：EKUAFU_TYPE）
PLAYER_INT_FIELD_MAIN_QUEST_ID = 596	-- 当前主线任务
PLAYER_INT_FIELD_QUEST_CHAPTER = 597	-- 任务章节奖励领取情况
PLAYER_INT_FIELD_MOUNT_LEVEL = 598	-- 4个bytes(0:当前坐骑阶数 1:当前坐骑星级,2:当前坐骑是否骑乘，3:当前幻化id)
PLAYER_INT_FIELD_DIVINE_ID = 599	-- 当前装备的神兵id
PLAYER_INT_FIELD_PASSIVE_SPELL_START = 600	-- 被动技能开始
PLAYER_INT_FIELD_PASSIVE_SPELL_END = 650	-- 被动技能个数
PLAYER_INT_FIELD_VIRTUAL_CAMP = 650	-- 虚拟阵营
PLAYER_INT_FIELD_WORLD_3V3_SCORE = 651	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TOTAL_SCORE = 652	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TREND_INFO = 653	-- 胜负走势
PLAYER_INT_FIELD_DOUJIANTAI_RANK = 654	-- 斗剑台排名
PLAYER_INT_FIELD_DOUJIANTAI_REFRESH_TIME = 655	-- 最后一次刷新对手时间戳
PLAYER_INT_FIELD_ARREARS = 656	-- 欠费
PLAYER_INT_FIELD_DAILY_QUEST_FINISHED = 658	--  每日任务完成数
PLAYER_INT_FIELD_BAG_SORT_TIME = 659	-- 整理背包时间戳
-- 已开启功能的记录
PLAYER_INT_FIELD_OPEN_MENU_INDEX = 660
PLAYER_INT_FIELD_OPEN_MENU_START = 661
PLAYER_INT_FIELD_OPEN_MENU_END = 761
-- 已购家族商店物品的记录
...
-- 修炼场相关
PLAYER_INT_FIELD_CULTIVATION_LAST_PLUNDER_TIME = 773	-- 上次挑战时间戳
PLAYER_INT_FIELD_CULTIVATION_LAST_REFRESH_TIME = 774	-- 上次刷新对手时间戳
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_EXP = 775	-- 当日累计掠夺经验
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_CHEST = 776	-- 当日累计掠夺宝箱数
PLAYER_INT_FIELD_CULTIVATION_TODAY_PURCHASE_COUNT = 777	-- 当日购买次数
PLAYER_INT_FIELD_CULTIVATION_LEFT_PLUNDER_COUNT = 778	-- 当前剩余挑战次数
PLAYER_INT_FIELD_CULTIVATION_PLUNDER_RECORD_INDEX = 779	-- 修练场修炼记录下标
PLAYER_INT_FIELD_CULTIVATION_TOTAL_BEEN_PLUNDER_COUNT = 780	-- 本轮被掠夺成功次数
PLAYER_INT_FIELD_CULTIVATION_TODAY_TOTAL_PLUNDER_COUNT = 781	-- 本日总掠夺次数
PLAYER_INT_FIELD_CULTIVATION_START_TIME = 782	-- 修炼开始时间
PLAYER_INT_FIELD_CULTIVATION_LOST_INFO = 783	-- 修炼宝箱丢失信息
-- ////////////////////////////////////////////////////////////////////////////////////////////////////////
-- 字符串部分
...
MODULE_ACTIVITY_DAILY_TASKS = 2	-- 日常任务
