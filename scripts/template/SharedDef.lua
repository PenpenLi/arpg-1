...
UINT_FIELD_BOOS_CUR_SPELLID = 131	-- 当前正在释放的蓄力技能idUNIT_INT_FIELD_APPEARANCE
...
UNIT_INT_FIELD_APPEARANCE = 235	--  外观
MAX_UNIT_FIELD = 236
...
PLAYER_APPD_INT_FIELD_RESET_WEEK = 553	--  每周重置时间
PLAYER_APPD_INT_FIELD_RESET_DALIY = 554	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 578	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 579	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 580	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 588	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 589	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 590	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 592	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 593	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 594	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
-- 场景服扩展下标开始了
PLAYER_SCENED_INT_FLAGS = 595	-- 玩家场景服标志位
--  只显示装备的主动技能
PLAYER_INT_FIELD_SPELL_START = 603
PLAYER_INT_FIELD_SPELL_END = 615
-- 重要技能的CD
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_START = 615
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_END = 655
PLAYER_EXPAND_KILL_MONSTER = 655	-- 杀怪数
-- 跨服
PLAYER_APPD_INT_FIELD_KUAFU_WARID = 656	-- 跨服场次id
PLAYER_INT_FIELD_KUAFU_NUMBER = 657	-- 0:玩家报名跨服时收到匹配信息中给的编号 1:跨服类型（值参照枚举：EKUAFU_TYPE）
PLAYER_INT_FIELD_MAIN_QUEST_ID = 658	-- 当前主线任务
PLAYER_INT_FIELD_QUEST_CHAPTER = 659	-- 任务章节奖励领取情况
PLAYER_INT_FIELD_MOUNT_LEVEL = 660	-- 4个bytes(0:当前坐骑阶数 1:当前坐骑星级,2:当前坐骑是否骑乘，3:当前幻化id)
PLAYER_INT_FIELD_DIVINE_ID = 661	-- 当前装备的神兵id
PLAYER_INT_FIELD_PASSIVE_SPELL_START = 662	-- 被动技能开始
PLAYER_INT_FIELD_PASSIVE_SPELL_END = 762	-- 被动技能个数
PLAYER_INT_FIELD_VIRTUAL_CAMP = 762	-- 虚拟阵营
PLAYER_INT_FIELD_WORLD_3V3_SCORE = 763	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TOTAL_SCORE = 764	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TREND_INFO = 765	-- 胜负走势
PLAYER_INT_FIELD_DOUJIANTAI_RANK = 766	-- 斗剑台排名
PLAYER_INT_FIELD_DOUJIANTAI_REFRESH_TIME = 767	-- 最后一次刷新对手时间戳
PLAYER_INT_FIELD_ARREARS = 768	-- 欠费
PLAYER_INT_FIELD_DAILY_QUEST_FINISHED = 770	--  每日任务完成数
PLAYER_INT_FIELD_BAG_SORT_TIME = 771	-- 整理背包时间戳
-- 已开启功能的记录
PLAYER_INT_FIELD_OPEN_MENU_INDEX = 772
PLAYER_INT_FIELD_OPEN_MENU_START = 773
PLAYER_INT_FIELD_OPEN_MENU_END = 873
-- 已购家族商店物品的记录
PLAYER_INT_FIELD_BUYED_FACTION_SHOP = 874	-- 已购家族商店物品开始
PLAYER_INT_FIELD_BUYED_FACTION_SHOP_END = 882	-- 已购家族商店物品结束
-- 记录强制引导ID 
PLAYER_INT_FIELD_GUIDE_ID_LAST = 883	-- 上次完成的引导
PLAYER_INT_FIELD_GUIDE_ID_NOW = 884	-- 当前进行中的引导
-- 修炼场相关
PLAYER_INT_FIELD_CULTIVATION_LAST_PLUNDER_TIME = 885	-- 上次挑战时间戳
PLAYER_INT_FIELD_CULTIVATION_LAST_REFRESH_TIME = 886	-- 上次刷新对手时间戳
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_EXP = 887	-- 当日累计掠夺经验
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_CHEST = 888	-- 当日累计掠夺宝箱数
PLAYER_INT_FIELD_CULTIVATION_TODAY_PURCHASE_COUNT = 889	-- 当日购买次数
PLAYER_INT_FIELD_CULTIVATION_LEFT_PLUNDER_COUNT = 890	-- 当前剩余挑战次数
PLAYER_INT_FIELD_CULTIVATION_PLUNDER_RECORD_INDEX = 891	-- 修练场修炼记录下标
PLAYER_INT_FIELD_CULTIVATION_TOTAL_BEEN_PLUNDER_COUNT = 892	-- 本轮被掠夺成功次数
PLAYER_INT_FIELD_CULTIVATION_TODAY_TOTAL_PLUNDER_COUNT = 893	-- 本日总掠夺次数
PLAYER_INT_FIELD_CULTIVATION_START_TIME = 894	-- 修炼开始时间
PLAYER_INT_FIELD_CULTIVATION_LOST_INFO = 895	-- 修炼宝箱丢失信息
-- 登陆大礼相关
PLAYER_INT_FIELD_LOGIN_ACTIVITY_TOTAL_DAYS = 896	-- 登陆总天数
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_START = 897	-- 领取状态开始
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_END = 899	-- 领取状态结束
--  非强制引导ID
PLAYER_INT_FIELD_OPTIONAL_GUIDE_START = 900	-- 非强制引导开始
PLAYER_INT_FIELD_OPTIONAL_GUIDE_END = 960	--  非强制引导结束
PLAYER_INT_FILED_LEAVE_RISK_TIME = 960	-- 离开冒险世界的时间戳
PLAYER_INT_FILED_LEAVE_RISK_PICKED = 961	-- 是否已经领取不在冒险世界的挂机奖励
PLAYER_INT_FILED_FACTION_BUILDING_SPEEDUP_DAILY_COUNT = 962	-- 家族建筑升级加速每日次数
PLAYER_INT_FILED_FACTION_DONATE_GIFT_EXCHANGE_DAILY_COUNT = 963	-- 家族宝库贡献礼物兑换每日次数
PLAYER_INT_FILED_CHARM_POINT = 964	-- 魅力值
PLAYER_INT_FILED_FACTION_GIFT_NEXT_COUNT_ID = 966	-- 玩家礼物历史记录下一个count_id
PLAYER_INT_FILED_FACTION_GIFT_START = 967	-- 玩家礼物历史记录开始
PLAYER_INT_FILED_FACTION_GIFT_END = 1717
PLAYER_INT_FIELD_CREATE_ICON = 1717	-- 待创建的帮派icon
PLAYER_INT_FIELD_FACTION_GIFT_POINT_COUNT = 1718	-- 帮派魅力赠送总记数
PLAYER_INT_FIELD_FACTION_GIFT_SEND_COUNT = 1720	-- 帮派礼物赠送总记数
PLAYER_INT_FIELD_FACTION_GIFT_BEEN_THANK_COUNT = 1721	-- 帮派礼物赠送被女王感谢记数
PLAYER_INT_FIELD_MASS_BOSS_BUYED_TIMES = 1722	-- 已经购买次数
PLAYER_INT_FIELD_MASS_BOSS_TIMES = 1723	-- 全民boss次数
PLAYER_INT_FIELD_MASS_BOSS_CD = 1724	-- 全民boss回复cd(0:表示已满了, 只有次数不满的情况才有cd)
PLAYER_INT_FIELD_TALISMAN_FORCE = 1725	-- 法宝总战力
PLAYER_INT_FIELD_WINGS_FORCE = 1726	-- 神羽总战力
PLAYER_INT_FIELD_GROUP_INSTANCE_CLEAR_FLAG = 1727	-- 组队副本首次通关flag
PLAYER_INT_FIELD_MERIDIAN_FORCE = 1728	-- 经脉修炼战力
PLAYER_INT_FIELD_WINGS_RANK = 1729	-- 神羽阶数
PLAYER_INT_FIELD_FACTION_BOSSDEFENSE_TICKETS = 1730	-- 家族首领挑战剩余次数
PLAYER_INT_FIELD_FACTION_TOWER_CLEAR_FLOOR = 1731	-- 家族无尽远征 历史挑战层数
PLAYER_INT_FIELD_FACTION_TOWER_FLAG = 1732	-- 0 扫荡状态 1~15 宝箱领取状态
PLAYER_INT_FIELD_QUALIFY_SCORE = 1733	--  排位赛积分
PLAYER_INT_FIELD_FACTION_SKILL_LV_START = 1734	-- 家族技能等级开始 0:已学习等级 uint16 1:当前生效等级
PLAYER_INT_FIELD_FACTION_SKILL_LV_END = 1754	-- 家族技能等级结束
PLAYER_INT_FIELD_APPEARANCE = 1754	--  外观(0:武器,1:衣服)
PLAYER_INT_FIELD_WEAPON_FORCE = 1755	--  武器外观战力
PLAYER_INT_FIELD_CLOTH_FORCE = 1756	--  衣服外观战力
...
INSTANCE_INT_FIELD_QUALIFY_EXTRA = 80	--  排位赛胜利额外可领取奖励标志
INSTANCE_INT_FIELD_QUALIFY_WINS = 81	--  排位赛赢得场次		
INSTANCE_INT_FIELD_QUALIFY_EXTRA_PICKED = 82	--  排位赛胜利额外已领取奖励标志
INSTANCE_INT_FIELD_QUALIFY_DAILY_TIMES = 83	--  排位赛每天次数
INSTANCE_INT_FIELD_QUALIFY_BUY_TIMES = 84	--  排位赛购买次数
INSTANCE_INT_FIELD_QUALIFY_CURSOR = 85	--  排位赛记录游标
