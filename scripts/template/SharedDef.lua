...
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 290	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 291	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 292	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 293	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 294	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 295	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 296	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 320	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 321	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 323	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 325	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 326	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 327	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 328	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 329	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 330
PLAYER_EXPAND_INT_TO_LINE_NO = 331	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 332	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 333	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 334
PLAYER_EXPAND_INT_CLIENT_DATA = 335	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 435	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 442	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 442	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 443	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 467	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 468	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 469	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 477	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 478	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 479	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 481	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 482	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 483	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
...
RANK_LIST_CURRENT_OBJ_INT_FIELD_TRIAL = 9	-- 试炼塔层数
RANK_LIST_CURRENT_OBJ_INT_FIELD_SHOW = 10	-- 外观列表开始
RANK_LIST_CURRENT_OBJ_INT_FIELD_SHOW_END = 14	-- 外观结束
