...
UNIT_FIELD_SELF_DEFENSE_GUID = 9	-- 自卫模式下可以攻击的玩家intguid
UNIT_FIELD_FLAGS = 10	-- 精灵身上的标志位
GO_FIELD_FLAGS = 11	-- 游戏对象的一些标识位
GO_FIELD_DATA = 12	-- 动态数据字段,目前保留为4个int
UNIT_FIELD_NPC_FLAG = 16	-- NPC标识
UNIT_FIELD_INSTANCE_ID = 17	-- 地图实例ID
...
UNIT_FIELD_HEALTH = 73	-- 当前生命	
UNIT_FIELD_MAXHEALTH = 74	-- 最大生命
UNIT_FIELD_DAMAGE = 75	-- 攻击力
UNIT_FIELD_ARMOR = 76	-- 防御力
UNIT_FIELD_HIT = 77	-- 命中
UNIT_FIELD_DODGE = 78	-- 闪避
UNIT_FIELD_CRIT = 79	-- 暴击
UNIT_FIELD_TOUGH = 80	-- 坚韧
UNIT_FIELD_ATTACK_SPEED = 81	-- 攻击速度
UNIT_FIELD_MOVE_SPEED = 82	-- 移动速度
UNIT_FIELD_AMPLIFY_DAMAGE = 83	-- 伤害加深(万分比)
UNIT_FIELD_IGNORE_DEFENSE = 84	-- 忽视防御(万分比)
UNIT_FIELD_DAMAGE_RESIST = 85	-- 伤害减免(万分比)
UNIT_FIELD_DAMAGE_RETURNED = 86	-- 反弹伤害(万分比)
UNIT_FIELD_HIT_RATE = 87	-- 命中率加成(万分比)
UNIT_FIELD_DODGE_RATE = 88	-- 闪避率加成(万分比)
UNIT_FIELD_CRIT_RATE = 89	-- 暴击率加成(万分比)
UNIT_FIELD_CRITICAL_RESIST_RATE = 90	-- 抗暴率加成(万分比)
UNIT_FIELD_DAMAGE_CRIT_MULTIPLE = 91	-- 暴击伤害倍数(万分比)
UNIT_FIELD_RESIST_CRIT_MULTIPLE = 92	-- 降暴伤害倍数(万分比)
UNIT_FIELD_ANGER = 93	-- 愤怒值
UNIT_FIELD_PEACE_MODE_CD = 94	-- 和平模式CD
UNIT_FIELD_NOTORIETY = 95	-- 2个short(0:战斗模式, 1:恶名值)
...
-- 游戏对象数据
-- 传送点专用
GO_TELETEPORTER_TO_MAPID = 12
GO_TELETEPORTER_TO_X = 13
GO_TELETEPORTER_TO_Y = 14
-- 任务相关游戏对象
GO_QUEST_ID_0 = 12	-- 相关任务0
GO_QUEST_ID_1 = 13	-- 相关任务1
GO_QUEST_ID_2 = 14	-- 相关任务2
GO_QUEST_ID_3 = 15	-- 相关任务3
GO_STRING_FIELD_CREATED_NAME = 5	-- 战利品创建者Name
-- 机关类相关的
GO_GEAR_STATUS = 12	-- 机关状态
GO_GEAR_KILLER = 13	-- 机关破坏者
-- 机关状态枚举
...
PLAYER_FIELD_BYTES_3 = 261	-- 四个字节,分别存放 0头像,[1发型2可用技能槽数量3攻击模式]
...
PLAYER_FIELD_VIP1_TIME_OUT = 268	-- VIP1 超时时间
PLAYER_FIELD_VIP2_TIME_OUT = 269	-- VIP2 超时时间
PLAYER_FIELD_VIP3_TIME_OUT = 270	-- VIP3 超时时间
PLAYER_FIELD_FLAGS = 271	-- 主玩家下标flags
PLAYER_FIELD_FLAGS_END = 279	-- 预留8个32位flags
PLAYER_FIELD_ONLINE_TIME = 279	-- 总在线时长(分钟为单位)
PLAYER_FIELD_ANGER = 280	-- 怒气改变
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 281	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 282	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 283	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 284	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 285	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 286	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 287	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 311	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 312	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 314	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 316	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 317	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 318	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 319	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 320	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 321
PLAYER_EXPAND_INT_TO_LINE_NO = 322	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 323	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 324	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 325
PLAYER_EXPAND_INT_CLIENT_DATA = 326	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 426	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 433	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 433	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 434	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 458	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 459	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 460	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 468	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 469	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 470	-- 主包裹扩展剩余多少时间
