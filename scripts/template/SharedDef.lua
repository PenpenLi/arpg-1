...
PLAYER_FIELD_ANGER = 278	-- 怒气改变
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 279	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 280	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 281	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 282	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 283	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 284	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 285	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 309	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 310	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 312	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 314	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 315	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 316	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 317	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 318	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 319
PLAYER_EXPAND_INT_TO_LINE_NO = 320	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 321	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 322	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 323
PLAYER_EXPAND_INT_CLIENT_DATA = 324	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 424	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 431	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 431	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 432	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 456	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 457	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 458	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 466	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 467	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 468	-- 主包裹扩展剩余多少时间
