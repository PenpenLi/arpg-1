...
PLAYER_SCENED_INT_FIELD_PLAYER_JUMP_CD = 285	-- 跳跃CD
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 286	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 287	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 288	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 289	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 290	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 291	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 292	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 316	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 317	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 319	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 321	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 322	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 323	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 324	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 325	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 326
PLAYER_EXPAND_INT_TO_LINE_NO = 327	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 328	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 329	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 330
PLAYER_EXPAND_INT_CLIENT_DATA = 331	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 431	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 438	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 438	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 439	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 463	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 464	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 465	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 473	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 474	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 475	-- 主包裹扩展剩余多少时间
