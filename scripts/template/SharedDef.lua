...
QUEST_TARGET_TYPE_TALISMAN_OWNS = 72	-- 拥有法宝X个
-- 货币处理类型
...
PLAYER_EXPAND_INT_MONEY_END = 430
...
PLAYER_EXPAND_INT_XP_ADDED = 435	--  应用服增加的经验
PLAYER_EXPAND_INT_XP_PICKED = 437	--  场景服已经领取的应用服增加的经验
PLAYER_EXPAND_INT_MAP_STATUS = 439	--  地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 440	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 441	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 442	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 443	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 444
PLAYER_EXPAND_INT_TO_LINE_NO = 445	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 446	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 447	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 448
PLAYER_EXPAND_INT_CLIENT_DATA = 449	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 549	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 556	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 556	--  每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_WEEK = 557	--  每周重置时间
PLAYER_APPD_INT_FIELD_RESET_DALIY = 558	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 582	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 583	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 584	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 592	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 593	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 594	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 596	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 597	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 598	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
-- 场景服扩展下标开始了
PLAYER_SCENED_INT_FLAGS = 599	-- 玩家场景服标志位
--  只显示装备的主动技能
PLAYER_INT_FIELD_SPELL_START = 607
PLAYER_INT_FIELD_SPELL_END = 619
-- 重要技能的CD
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_START = 619
PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_END = 659
PLAYER_EXPAND_KILL_MONSTER = 659	-- 杀怪数
-- 跨服
PLAYER_APPD_INT_FIELD_KUAFU_WARID = 660	-- 跨服场次id
PLAYER_INT_FIELD_KUAFU_NUMBER = 661	-- 0:玩家报名跨服时收到匹配信息中给的编号 1:跨服类型（值参照枚举：EKUAFU_TYPE）
PLAYER_INT_FIELD_MAIN_QUEST_ID = 662	-- 当前主线任务
PLAYER_INT_FIELD_QUEST_CHAPTER = 663	-- 任务章节奖励领取情况
PLAYER_INT_FIELD_MOUNT_LEVEL = 664	-- 4个bytes(0:当前坐骑阶数 1:当前坐骑星级,2:当前坐骑是否骑乘，3:当前幻化id)
PLAYER_INT_FIELD_DIVINE_ID = 665	-- 当前装备的神兵id
PLAYER_INT_FIELD_PASSIVE_SPELL_START = 666	-- 被动技能开始
PLAYER_INT_FIELD_PASSIVE_SPELL_END = 766	-- 被动技能个数
PLAYER_INT_FIELD_VIRTUAL_CAMP = 766	-- 虚拟阵营
PLAYER_INT_FIELD_WORLD_3V3_SCORE = 767	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TOTAL_SCORE = 768	-- 跨服3v3积分
PLAYER_INT_FIELD_WORLD_3V3_TREND_INFO = 769	-- 胜负走势
PLAYER_INT_FIELD_DOUJIANTAI_RANK = 770	-- 斗剑台排名
PLAYER_INT_FIELD_DOUJIANTAI_REFRESH_TIME = 771	-- 最后一次刷新对手时间戳
PLAYER_INT_FIELD_ARREARS = 772	-- 欠费
PLAYER_INT_FIELD_DAILY_QUEST_FINISHED = 774	--  每日任务完成数
PLAYER_INT_FIELD_BAG_SORT_TIME = 775	-- 整理背包时间戳
-- 已开启功能的记录
PLAYER_INT_FIELD_OPEN_MENU_INDEX = 776
PLAYER_INT_FIELD_OPEN_MENU_START = 777
PLAYER_INT_FIELD_OPEN_MENU_END = 877
-- 已购家族商店物品的记录
PLAYER_INT_FIELD_BUYED_FACTION_SHOP = 878	-- 已购家族商店物品开始
PLAYER_INT_FIELD_BUYED_FACTION_SHOP_END = 886	-- 已购家族商店物品结束
-- 记录强制引导ID 
PLAYER_INT_FIELD_GUIDE_ID_LAST = 887	-- 上次完成的引导
PLAYER_INT_FIELD_GUIDE_ID_NOW = 888	-- 当前进行中的引导
-- 修炼场相关
PLAYER_INT_FIELD_CULTIVATION_LAST_PLUNDER_TIME = 889	-- 上次挑战时间戳
PLAYER_INT_FIELD_CULTIVATION_LAST_REFRESH_TIME = 890	-- 上次刷新对手时间戳
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_EXP = 891	-- 当日累计掠夺经验
PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_CHEST = 892	-- 当日累计掠夺宝箱数
PLAYER_INT_FIELD_CULTIVATION_TODAY_PURCHASE_COUNT = 893	-- 当日购买次数
PLAYER_INT_FIELD_CULTIVATION_LEFT_PLUNDER_COUNT = 894	-- 当前剩余挑战次数
PLAYER_INT_FIELD_CULTIVATION_PLUNDER_RECORD_INDEX = 895	-- 修练场修炼记录下标
PLAYER_INT_FIELD_CULTIVATION_TOTAL_BEEN_PLUNDER_COUNT = 896	-- 本轮被掠夺成功次数
PLAYER_INT_FIELD_CULTIVATION_TODAY_TOTAL_PLUNDER_COUNT = 897	-- 本日总掠夺次数
PLAYER_INT_FIELD_CULTIVATION_START_TIME = 898	-- 修炼开始时间
PLAYER_INT_FIELD_CULTIVATION_LOST_INFO = 899	-- 修炼宝箱丢失信息
-- 登陆大礼相关
PLAYER_INT_FIELD_LOGIN_ACTIVITY_TOTAL_DAYS = 900	-- 登陆总天数
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_START = 901	-- 领取状态开始
PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_END = 903	-- 领取状态结束
--  非强制引导ID
PLAYER_INT_FIELD_OPTIONAL_GUIDE_START = 904	-- 非强制引导开始
PLAYER_INT_FIELD_OPTIONAL_GUIDE_END = 964	--  非强制引导结束
PLAYER_INT_FILED_LEAVE_RISK_TIME = 964	-- 离开冒险世界的时间戳
PLAYER_INT_FILED_LEAVE_RISK_SUIT_SCORE = 965	-- 是否已经领取不在冒险世界的挂机奖励
PLAYER_INT_FILED_FACTION_BUILDING_SPEEDUP_DAILY_COUNT = 966	-- 家族建筑升级加速每日次数
PLAYER_INT_FILED_FACTION_DONATE_GIFT_EXCHANGE_DAILY_COUNT = 967	-- 家族宝库贡献礼物兑换每日次数
PLAYER_INT_FILED_CHARM_POINT = 968	-- 魅力值
PLAYER_INT_FILED_FACTION_GIFT_NEXT_COUNT_ID = 970	-- 玩家礼物历史记录下一个count_id
PLAYER_INT_FILED_FACTION_GIFT_START = 971	-- 玩家礼物历史记录开始
PLAYER_INT_FILED_FACTION_GIFT_END = 1721
PLAYER_INT_FIELD_CREATE_ICON = 1721	-- 待创建的帮派icon
PLAYER_INT_FIELD_FACTION_GIFT_POINT_COUNT = 1722	-- 帮派魅力赠送总记数
PLAYER_INT_FIELD_FACTION_GIFT_SEND_COUNT = 1724	-- 帮派礼物赠送总记数
PLAYER_INT_FIELD_FACTION_GIFT_BEEN_THANK_COUNT = 1725	-- 帮派礼物赠送被女王感谢记数
PLAYER_INT_FIELD_MASS_BOSS_BUYED_TIMES = 1726	-- 已经购买次数
PLAYER_INT_FIELD_MASS_BOSS_TIMES = 1727	-- 全民boss次数
PLAYER_INT_FIELD_MASS_BOSS_CD = 1728	-- 全民boss回复cd(0:表示已满了, 只有次数不满的情况才有cd)
PLAYER_INT_FIELD_TALISMAN_FORCE = 1729	-- 法宝总战力
PLAYER_INT_FIELD_WINGS_FORCE = 1730	-- 神羽总战力
PLAYER_INT_FIELD_GROUP_INSTANCE_CLEAR_FLAG = 1731	-- 组队副本首次通关flag
PLAYER_INT_FIELD_MERIDIAN_FORCE = 1732	-- 经脉修炼战力
PLAYER_INT_FIELD_WINGS_RANK = 1733	-- 神羽阶数
PLAYER_INT_FIELD_FACTION_BOSSDEFENSE_TICKETS = 1734	-- 家族首领挑战剩余次数
PLAYER_INT_FIELD_FACTION_TOWER_CLEAR_FLOOR = 1735	-- 家族无尽远征 历史挑战层数
PLAYER_INT_FIELD_FACTION_TOWER_FLAG = 1736	-- 0 扫荡状态 1~15 宝箱领取状态
PLAYER_INT_FIELD_QUALIFY_SCORE = 1737	--  排位赛积分
PLAYER_INT_FIELD_FACTION_SKILL_LV_START = 1738	-- 家族技能等级开始 0:已学习等级 uint16 1:当前生效等级
PLAYER_INT_FIELD_FACTION_SKILL_LV_END = 1758	-- 家族技能等级结束
PLAYER_INT_FIELD_APPEARANCE = 1758	--  外观(0:武器,1:衣服)
PLAYER_INT_FIELD_WEAPON_FORCE = 1759	--  武器外观战力
PLAYER_INT_FIELD_CLOTH_FORCE = 1760	--  衣服外观战力
PLAYER_INT_FIELD_POKEDEX_FORCE = 1761	--  图鉴战力
PLAYER_INT_FIELD_TITLE_FORCE = 1762	--  称号战力
PLAYER_INT_FIELD_RENAME_TIMES = 1763	--  改名次数
PLAYER_INT_FIELD_REVENGE_TIMES = 1764	-- 当日剩余复仇次数
PLAYER_INT_FIELD_REVENGE_BUY_TIMES = 1765	-- 当日复仇购买次数
PLAYER_INT_FIELD_WINGS_STAR = 1766	-- 神羽星数
PLAYER_INT_FIELD_VIPGIFT_FLAG = 1767	-- vip礼包购买状态
PLAYER_APPD_INT_FIELD_CONSUME_SUM = 1768	-- 累计消费元宝
-- ////////////////////////////////////////////////////////////////////////////////////////////////////////
-- 字符串部分
...
MODULE_FIRST_RECHARGE_ALL = 1	-- 首冲hu
