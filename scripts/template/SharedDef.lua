...
UNIT_FIELD_EQUIPMENT_COAT = 16	-- 上衣模板
UNIT_FIELD_EQUIPMENT_MAIN_WEAPON = 17	-- 主手武器
UNIT_FIELD_BUFF = 18	-- BUFFID
-- 注意了：创建包需要下发的下标放在 UNIT_FIELD_BUFF前面,UNIT_FIELD_BUFF_TM后面的下标走订阅
UNIT_FIELD_BUFF_TM = 26	-- BUFF时间（掩码从这开始）
UNIT_FIELD_BUFF_RESERVE = 34	-- BUFF预留值
UNIT_FIELD_BUFF_LEVEL = 50	-- BUFF等级（byte）
UNIT_FIELD_BUFF_CASTER_GUID = 54	-- buff施法者guid
UNIT_FIELD_FORCE = 70	-- 战斗力
UNIT_FIELD_HEALTH = 71	-- 当前生命	
UNIT_FIELD_MAXHEALTH = 72	-- 最大生命
UNIT_FIELD_DAMAGE = 73	-- 攻击力
UNIT_FIELD_ARMOR = 74	-- 防御力
UNIT_FIELD_HIT = 75	-- 命中
UNIT_FIELD_DODGE = 76	-- 闪避
UNIT_FIELD_CRIT = 77	-- 暴击
UNIT_FIELD_TOUGH = 78	-- 坚韧
UNIT_FIELD_ATTACK_SPEED = 79	-- 攻击速度
UNIT_FIELD_MOVE_SPEED = 80	-- 移动速度
UNIT_FIELD_AMPLIFY_DAMAGE = 81	-- 伤害加深(万分比)
UNIT_FIELD_IGNORE_DEFENSE = 82	-- 忽视防御(万分比)
UNIT_FIELD_DAMAGE_RESIST = 83	-- 伤害减免(万分比)
UNIT_FIELD_DAMAGE_RETURNED = 84	-- 反弹伤害(万分比)
UNIT_FIELD_HIT_RATE = 85	-- 命中率加成(万分比)
UNIT_FIELD_DODGE_RATE = 86	-- 闪避率加成(万分比)
UNIT_FIELD_CRIT_RATE = 87	-- 暴击率加成(万分比)
UNIT_FIELD_CRITICAL_RESIST_RATE = 88	-- 抗暴率加成(万分比)
UNIT_FIELD_DAMAGE_CRIT_MULTIPLE = 89	-- 暴击伤害倍数(万分比)
UNIT_FIELD_RESIST_CRIT_MULTIPLE = 90	-- 降暴伤害倍数(万分比)
UNIT_FIELD_ANGER = 91	-- 愤怒值
UINT_FIELD_BOSS_DATA = 92	-- boss数据byte 0:boss状态 1:boss前一状态 2:boss索引(云游的是这个用途)
UINT_FIELD_BOSS_DATA1 = 93	-- boss数据byte 0:当前坐标x 1:当前坐标y 2:下一坐标x 3:下一坐标y(云游的是这个用途)
MAX_UNIT_FIELD = 94
...
PLAYER_INT_FIELD_DIVINE_ID = 565	-- 当前装备的神兵id
PLAYER_INT_FIELD_DIVINE_INITIATIVE = 566	-- 神兵主动技能信息
PLAYER_INT_FIELD_DIVINE_PASSIVE_START = 567	-- 神兵被动技能开始
PLAYER_INT_FIELD_DIVINE_PASSIVE_END = 570	-- 神兵被动技能结束
PLAYER_INT_FIELD_PASSIVE_SPELL_COUNT = 570	-- 被动技能的个数
...
SPELL_DIVINE_COUNT = 31	-- 神兵个数
SPELL_INT_FIELD_DIVINE_START = 32	-- 神兵开始
SPELL_INT_FIELD_DIVINE_END = 82
-- 需要发内部消息同步到场景服的技能类型
