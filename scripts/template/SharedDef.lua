...
INSTANCE_INT_FIELD_RES_START = 4	-- 资源副本开始 每个信息4个byte[0:挑战次数,1:预留,2:预留,3:预留]
...
MAP_INT_FIELD_QUESTS_START = 6	-- 副本任务开始
MAP_INT_FIELD_QUESTS_END = 30	-- 副本任务结束
MAP_INT_FIELD_QUESTS_PROCESS_START = 30	-- 进度开始
MAP_INT_FIELD_QUESTS_PROCESS_END = 33	-- 进度结束
MAP_INT_FIELD_CREATE_TM = 33	-- 副本创建时间
MAP_INT_FIELD_QUEST_END_TM = 34	-- 副本任务结束时间
MAP_INT_FIELD_END_TM = 35	-- 副本结束时间
MAP_INT_FIELD_INSTANCE_TYPE = 36	-- 副本类型
-- 地图字符串部分
MAP_STR_GENERAL_ID = 4
MAP_STR_REWARD = 5	-- 副本奖励物品字符串
-- VIP BOSS
VIP_INSTANCE_FIELD_ID = 37	--  0:VIP副本序号,1:副本难度
WORLDBOSS_FIELDS_WAIT_TIME = 37	-- 报名等待时间戳
WORLDBOSS_FIELDS_BORN_TIME = 38	-- 刷新等待时间戳
-- 试炼塔
TRIAL_INSTANCE_FIELD_ID = 37	--  当前层数
--  桃花迷阵最大刷新次数
TAOHUA_REFRESH_TIMES = 10
-- 桃花迷阵地图数据
TAOHUA_INT_FIELD_REFRESH_TIMES = 37	-- 桃花迷阵已经刷新次数
TAOHUA_INT_FIELD_BOSS_CURR_HP = 38	-- BOSS当前血量
TAOHUA_STR_FIELD_REAL_BOSS_GUID = 5	-- BOSS真身GUID
-- 塔的最大层数
TOWER_MAX_FLOOR = 2
-- 塔怪每层击杀数量
TAGUAI_INT_FIELD_FIRST_FLOOR = 37	-- 第一层的信息
-- 战斗信息整形字符型枚举
-- 貌似没必要,随便留一个吧
