...
UNIT_FIELD_PROCESS_TIME = 96	--  进度条读完的时间戳
UNIT_FIELD_PROCESS_SECONDS = 97	--  读条总共需要时间	
UINT_FIELD_BOSS_DATA = 98	-- boss数据byte 0:boss状态 1:boss前一状态 2:boss索引(云游的是这个用途)
UINT_FIELD_BOSS_DATA1 = 99	-- boss数据byte 0:当前坐标x 1:当前坐标y 2:下一坐标x 3:下一坐标y(云游的是这个用途)
MAX_UNIT_FIELD = 100
...
MAIL_SOURCE_LEVEL_RANK = 0	-- 来自排行榜
MAIL_SOURCE_WORLD_BOSS = 1	-- 来自世界BOSS
...
PLAYER_FIELD_BYTES_0 = 11	-- 四个字节,分别存放 0角色id,[1阵营2种族3职业]
PLAYER_FIELD_NPC_FLAG = 12	-- NPC标识
PLAYER_FIELD_DEATH_STATE = 13	-- 生存状态
PLAYER_FIELD_HEALTH = 14	-- 当前生命
PLAYER_FIELD_MAXHEALTH = 16	-- 最大生命
PLAYER_FIELD_DAMAGE = 18	-- 攻击力
PLAYER_FIELD_ARMOR = 20	-- 防御力
PLAYER_FIELD_HIT = 22	-- 命中
PLAYER_FIELD_DODGE = 24	-- 闪避
PLAYER_FIELD_CRIT = 26	-- 暴击
PLAYER_FIELD_TOUGH = 28	-- 坚韧
PLAYER_FIELD_ATTACK_SPEED = 30	-- 攻击速度
PLAYER_FIELD_MOVE_SPEED = 32	-- 移动速度
PLAYER_FIELD_AMPLIFY_DAMAGE = 34	-- 伤害加深(万分比)
PLAYER_FIELD_IGNORE_DEFENSE = 36	-- 忽视防御(万分比)
PLAYER_FIELD_DAMAGE_RESIST = 38	-- 伤害减免(万分比)
PLAYER_FIELD_DAMAGE_RETURNED = 40	-- 反弹伤害(万分比)
PLAYER_FIELD_HIT_RATE = 42	-- 命中率加成(万分比)
PLAYER_FIELD_DODGE_RATE = 44	-- 闪避率加成(万分比)
PLAYER_FIELD_CRIT_RATE = 46	-- 暴击率加成(万分比)
PLAYER_FIELD_CRITICAL_RESIST_RATE = 48	-- 抗暴率加成(万分比)
PLAYER_FIELD_DAMAGE_CRIT_MULTIPLE = 50	-- 暴击伤害倍数(万分比)
PLAYER_FIELD_RESIST_CRIT_MULTIPLE = 52	-- 降暴伤害倍数(万分比)
PLAYER_FIELD_ATTR_RESERVE0 = 54	-- 预留属性0
PLAYER_FIELD_ATTR_RESERVE1 = 56	-- 预留属性1
PLAYER_FIELD_ATTR_RESERVE2 = 58	-- 预留属性2
PLAYER_FIELD_ATTR_RESERVE3 = 60	-- 预留属性3
PLAYER_FIELD_ATTR_RESERVE4 = 62	-- 预留属性4
PLAYER_FIELD_ATTR_RESERVE5 = 64	-- 预留属性5
PLAYER_FIELD_ATTR_RESERVE6 = 66	-- 预留属性6
PLAYER_FIELD_ATTR_RESERVE7 = 68	-- 预留属性7
PLAYER_FIELD_BUFF = 70	-- BUFF
PLAYER_FIELD_BUFF_RESERVE = 120	-- BUFF保留参数	
PLAYER_INT_FIELD_BUFF_GIVER_UINT_GUID = 170	-- buff giver的uintGuid
PLAYER_INT_FIELD_BUFF_LEVEL_START = 220	-- buff等级 (byte)
PLAYER_INT_FIELD_BUFF_LEVEL_END = 233
PLAYER_FIELD_SKIN = 233	-- 生物变身
PLAYER_FIELD_BYTES_1 = 234	-- 四个字节,分别存放 0怪物分类
PLAYER_FIELD_FORCE = 235	-- 战斗力
PLAYER_FIELD_EQUIPMENT = 237	-- 装备列表，12个32位预留三个
PLAYER_FIELD_CUR_MOUNT = 257	-- 当前坐骑模板id
PLAYER_FIELD_PEACE_MODE_CD = 258	-- 和平模式CD
...
PLAYER_FIELD_VIP_LEVEL = 268	-- VIP 等级
PLAYER_FIELD_VIP_TIME_OUT = 269	-- VIP 超时时间
--  挂机设置
PLAYER_FIELD_HOOK = 270	-- 挂机状态
PLAYER_FIELD_HOOK_BYTE0 = 271	-- 0:槽1技能,1:槽2技能,2:槽3技能,3:槽4技能
PLAYER_FIELD_HOOK_BYTE1 = 272	-- 0:神兵技能,1:怒气技能2:使用银两买药,3:银两不足用元宝买药
PLAYER_FIELD_HOOK_SHORT = 273	-- 0:自动吃药百分比,1:自动使用道具id
PLAYER_FIELD_HOOK_BYTE3 = 274	-- 0:回程复活/原地复活,1:自动使用绑银购买复活丹,2:绑银不足用元宝,3:自动出售某种平直及以下装备
PLAYER_FIELD_FLAGS = 275	-- 主玩家下标flags
PLAYER_FIELD_FLAGS_END = 283	-- 预留8个32位flags
PLAYER_FIELD_ONLINE_TIME = 283	-- 总在线时长(分钟为单位)
PLAYER_FIELD_ANGER = 284	-- 怒气改变
PLAYER_FIELD_TRIAL_LAYERS = 285	-- 试炼塔层数
PLAYER_FIELD_DECLINE_CHANNEL_BYTE0 = 286	-- 拒绝频道A(0系统，1帮派，2组队，3世界)
PLAYER_FIELD_DECLINE_CHANNEL_BYTE1 = 287	-- 拒绝频道B(0:附近)
PLAYER_SCENED_INT_FIELD_PLAYER_JUMP_CD = 288	-- 跳跃CD
PLAYER_INT_FIELD_SYSTEM_MAIL_ID = 289	-- 当前领取的系统邮件的id
PLAYER_FIELD_DIVINE_NUM = 290	-- 神兵数量
PLAYER_FIELD_DIVINE_FORCE = 291	-- 神兵总战斗力
PLAYER_FIELD_MOUNT_BEST = 292	-- 最高坐骑信息(0:ID 1:等级 2：星数)
PLAYER_FIELD_MOUNT_FORCE = 293	-- 坐骑总战力
PLAYER_EXPAND_INT_FCM_LOGIN_TIME = 294	-- 防沉迷，单位：分钟	-1是非防沉迷玩家
PLAYER_EXPAND_INT_FCM_LOGOUT_TIME = 295	-- 防沉迷离线时间
PLAYER_EXPAND_INT_CREATE_TIME = 296	-- 角色创建时间
PLAYER_EXPAND_INT_LAST_LOGIN_TIME = 297	-- 最后登录时间
PLAYER_EXPAND_INT_LAST_LOGOUT_TIME = 298	-- 最后离线时间
PLAYER_EXPAND_INT_NUMBER_OF_LOGINS = 299	-- 累积登录次数
PLAYER_EXPAND_INT_MONEY = 300	-- 各种货币
PLAYER_EXPAND_INT_USE_FASHION = 324	-- 是否启用时装,目前用到20个pos
PLAYER_EXPAND_INT_XP = 325	-- 当前经验，64位
PLAYER_EXPAND_INT_NEXT_LEVEL_XP = 327	-- 下一级经验，64位
PLAYER_EXPAND_INT_MAP_STATUS = 329	-- 地图开启进度
PLAYER_EXPAND_INT_GAG_END_TIME = 330	-- 禁言结束时间
PLAYER_EXPAND_INT_TO_MAP = 331	-- 玩家要传送到的地图
PLAYER_EXPAND_INT_TO_INSTANCE_ID = 332	-- 玩家要传送到的地图实例ID
PLAYER_EXPAND_INT_TO_POS_X = 333	-- 玩家药传送到的坐标
PLAYER_EXPAND_INT_TO_POS_Y = 334
PLAYER_EXPAND_INT_TO_LINE_NO = 335	-- 传送线号
PLAYER_EXPAND_INT_DB_MAP = 336	-- 玩家最后一次进入副本前的地图	
PLAYER_EXPAND_INT_DB_POS_X = 337	-- 玩家最后一次进入副本前的坐标
PLAYER_EXPAND_INT_DB_POS_Y = 338
PLAYER_EXPAND_INT_CLIENT_DATA = 339	-- 远程存储
-- ////////////////////////////////////////////////////////////////////////
-- 应用服部分	
PLAYER_APPD_INT_FIELD_FLAG = 439	-- 玩家标志位
PLAYER_APPD_INT_FIELD_FLAG_END = 446	-- 结束下标
PLAYER_APPD_INT_FIELD_DAILY_TIME = 446	-- 每天在线时长（min）
PLAYER_APPD_INT_FIELD_RESET_DALIY = 447	-- 重置的时间点每小时一次
PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 471	-- 累计充值总数
PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 472	-- 最后一笔充值时间
PLAYER_APPD_INT_FLELD_JHM_FLAGS = 473	-- 激活码flags
PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 481	-- 激活码flags_end
-- 主背包扩展时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 482	-- 主包裹扩展结束时间戳
PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 483	-- 主包裹扩展剩余多少时间
PLAYER_INT_FIELD_BLOCK_COUNT = 485	-- 屏蔽列表个数
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 486	-- 最后一次参加的世界BOSS id
PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 487	-- 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
PLAYER_APPD_INT_FIELD_END = 500
-- 场景服扩展下标开始了
...
FACTION_STRING_FIELD_PLAYER = 5	-- 帮派成员列表		
FACTION_STRING_FIELD_PLAYER_END = 105	-- 帮派成员列表结束
FACTION_STRING_FIELD_APPLY_PLAYER = 105	-- 帮派申请玩家列表
FACTION_STRING_FIELD_APPLY_PLAYER_END = 111	-- 帮派申请玩家列表结束
FACTION_STRING_FIELD_EVENT = 111	-- 帮派记录开始
FACTION_STRING_FIELD_EVENT_END = 121	-- 帮派记录结束
FACTION_STRING_FIELD_GONGGAO = 121	-- 帮派公告
FACTION_STRING_FIELD_ZHAOMU_GONGGAO = 122	-- 招募公告
